...
@dp.callback_query(F.data == "more_grammar")
async def more_grammar_cb(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.delete()
    await _show_grammar_menu(callback.message, edit=False)
    await callback.answer()
...
async def more_feedback_cb(callback: types.CallbackQuery, state: FSMContext):
    ...
    class FakeMsg:
        ...
        chat = callback.message.chat
        async def answer(self, *a, **kw): return await callback.message.answer(*a, **kw)
    await feedback_menu(FakeMsg(), state)
    await callback.answer()
