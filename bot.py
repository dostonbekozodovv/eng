...
# Bellashuv natijalari yuboriladigan kanal ID (ENV dan o'qiladi)
import os as _os
BATTLE_RESULT_CHANNEL_ID = _os.getenv("BATTLE_RESULT_CHANNEL_ID", "")
if BATTLE_RESULT_CHANNEL_ID and BATTLE_RESULT_CHANNEL_ID.lstrip('-').isdigit():
    BATTLE_RESULT_CHANNEL_ID = int(BATTLE_RESULT_CHANNEL_ID)
else:
    BATTLE_RESULT_CHANNEL_ID = None
...
WEEKLY_BONUS_ENABLED: bool = True  # Admin tomonidan yoq/o'chir
...
# ── Global battle state ──────────────────────────────────────────────────
# Random matchmaking: { user_id: {"msg_id": int, "chat_id": int} }
BATTLE_QUEUE: dict = {}

# Aktiv bellashuvlar: { battle_id: { ... } }
# battle_id = min(uid1,uid2), max(uid1,uid2)
ACTIVE_BATTLES: dict = {}

# Do'st bellashuv takliflari: { token: { inviter_id, accepted_id|None, ... } }
FRIEND_BATTLE_INVITES: dict = {}

# Admin usernamelari (to'g'ri)
ADMIN_USERNAMES = {
    # user_id: "username"  — config.py dagi ADMIN_IDS bilan mos bo'lishi kerak
    # Quyida .env / config.py dagi birinchi admin sizning useringiz
    # Ikkinchi admin — akimingiz
    # Bular username_display uchun ishlatiladi, avtomatik aniqlanadi
}

# Foydalanuvchilar oxirgi faollik vaqti: { user_id: datetime }
USER_LAST_ACTIVE: dict = {}

# Bellashuv taklifi yuborilgan vaqt: { (inviter_id, opp_id): datetime }
FRIEND_BATTLE_INVITE_TIME: dict = {}

# Do'stlar ro'yxati: { user_id: set(friend_id, ...) }
FRIEND_LIST: dict = {}

# Foydalanuvchi yutish seriyasi (bot kuchini oshirish uchun)
...
async def subscription_middleware(handler, message: types.Message, data: dict):
    # Guruhda majburiy obuna so'ralmasin — faqat shaxsiy botda
...
async def cmd_start(message: types.Message, command: CommandObject = None):
    ...
    ref_id = None
    # Do'st bellashuv taklifi: ?start=fbattle_{inviter_id}
    if command and command.args and command.args.startswith("fbattle_"):
        parts_arg = command.args.split("_")
        if len(parts_arg) >= 2:
            try:
                inviter_id = int(parts_arg[1])
                if inviter_id != user_id:
                    get_or_create_user(user_id, name, username, None)
                    update_streak(user_id)
                    FRIEND_LIST.setdefault(inviter_id, set()).add(user_id)
                    FRIEND_LIST.setdefault(user_id, set()).add(inviter_id)
                    inviter_u = get_user(inviter_id)
                    iname = inviter_u.get("name") if inviter_u else "Kimdir"
                    kb = InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="✅ Qabul qilish", callback_data=f"fbaccept_{inviter_id}_{user_id}")],
                        [InlineKeyboardButton(text="❌ Rad qilish",  callback_data=f"fbdecline_{inviter_id}")],
                    ])
                    await message.answer(
    ...
    # Username o'zgargan bo'lsa DB ni yangilash
    if user and username and user.get("username") != username:
        try:
            conn = get_conn()
            cur  = conn.cursor()
    ...
    # Referral — faqat kanalga obuna bo'lgandan keyin hisoblanadi (check_sub da)
    # Bu yerda faqat ref_id ni saqlaymiz, ball berilmaydi
...
async def check_sub_cb(callback: types.CallbackQuery):
    ...
    if await check_subscription(user_id):
        ...
        # Referal allaqachon /start da hisoblanadi — bu yerda faqat belgi qo'yamiz
        ref_id = user.get("referred_by") if user else None
        if ref_id and not user.get("referral_notified"):
            try:
                conn = get_conn()
                cur  = conn.cursor()
                # Referral ballini faqat shu yerda (obuna tasdiqlanganda) beramiz
                cur.execute(
                ...
                cur.close()
                conn.close()
                add_score(int(ref_id), REFERRAL_BONUS)
                ref_user = get_user(user_id)
                ref_name = ref_user.get("name", "Do'stingiz") if ref_user else "Do'stingiz"
                await bot.send_message(
                    int(ref_id),
                    f"🎉 <b>Yangi referal!</b>\n\n"
                    f"👤 Siz taklif qilgan <b>{ref_name}</b> kanalga obuna bo'ldi!\n"
                    f"━━━━━━━━━━━━━━━\n"
                    f"⭐️ <b>+{REFERRAL_BONUS} ball</b> hisobingizga qo'shildi!"
                )
            except Exception:
                pass
...
async def cmd_streak(message: types.Message):
    ...
    if not user:
        ...
        return
    # show_streak funksiyasini qayta ishlatamiz
    await show_streak(message)
...
async def cmd_rating(message: types.Message):
    ...
    for i, u in enumerate(top):
        ...
        score = u.get("score", 0)
        text += f"{medals[i]} {link} — {score} ball\n"
...
async def show_referral(message: types.Message):
    ...
    text = (
        ...
        f"10 000 ball + kamida 1 ta referal → {PRIZE_SCORE_AMOUNT:,} so'm!"
    )
    kb_rows = [[
...
async def _send_next_word(target, user_id: int, edit: bool = False):
    ...
    # So'z o'rganish uchun: faqat birinchi marta (unlearned listda bor ekan, hali o'rganilmagan)
    already_learned = word_data["word"] in learned
    add_learned_word(user_id, word_data["word"])
    if not already_learned:
        add_score(user_id, WORD_LEARN_BONUS)
    ...
    ball_line = f"\n⭐️ +{WORD_LEARN_BONUS} ball qo'shildi!" if not already_learned else ""
    txt = (
        ...
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"💬 <i>{word_data['example']}</i>{ball_line}"
    )
    if edit:
        await target.edit_text(txt, reply_markup=kb)
    else:
...
async def group_quiz_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if chosen == correct:
        correct_count += 1
        _level = data.get("level", "beginner")
        _ball  = LEVEL_TEST_BONUS.get(_level, 10)
        # So'z o'rganish bo'limidan kirgan test (group_quiz=True, from_learn bo'lsa)
        if data.get("from_learn"):
            _group_num = data.get("group_num", 1)
            already_passed = _group_num in PASSED_GROUPS.get(user_id, set())
            if not already_passed:
                add_score(user_id, _ball)
                await _check_score_prize(user_id)
                await callback.answer(f"✅ To'g'ri! +{_ball} ball 🎉")
            else:
                await callback.answer("✅ To'g'ri! 🎉")
        else:
            # Test bo'limidan kirgan guruh testi — ball berilmaydi
            await callback.answer("✅ To'g'ri! 🎉")
    else:
        add_score(user_id, -25)
        await callback.answer(f"❌ Noto'g'ri! To'g'ri: {correct}\n⚠️ -25 ball", show_alert=True)
    ...
    if q_num >= total:
        ...
        passed  = percent >= 70
        _level  = data.get("level", "beginner")
        _ball   = LEVEL_TEST_BONUS.get(_level, 5)

        if _level == "beginner":
            level_tip = (
                f"\n\n💡 <b>Eslatma:</b> Oddiy darajada <b>+{_ball} ball/savol</b> — kam!\n"
                f"⬆️ <b>O'rta (+20)</b> yoki <b>Yuqori (+35)</b> darajaga o'ting!"
            )
        else:
            level_tip = ""
        ...
        if passed:
            ...
            update_user_group(user_id, new_group)
            # Bu guruhni o'tilgan deb belgilaymiz — keyingi ishlashda ball berilmaydi
            if data.get("from_learn"):
                PASSED_GROUPS.setdefault(user_id, set()).add(group_num)
            ...
            await callback.message.edit_text(
                ...
                f"🔓 <b>{new_group}-guruh ochildi!</b>\n"
                f"🚀 Yangi so'zlar sizi kutmoqda!{level_tip}",
                reply_markup=kb
            )
        else:
            kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            await callback.message.edit_text(
                ...
                f"💎 <i>VIP oling — testisiz o'ting!</i>{level_tip}",
                reply_markup=kb
            )
    else:
...
async def check_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if chosen == correct:
        correct_count += 1
        await callback.answer("✅ To'g'ri! 🎉")
        # Test bo'limida ball berilmaydi — faqat So'z o'rganish bo'limidagi testda beriladi
    else:
        add_score(user_id, -25)
        await callback.answer(f"❌ Noto'g'ri! To'g'ri: {correct}\n⚠️ -25 ball", show_alert=True)
    ...
    if q_num >= total:
        ...
        await callback.message.edit_text(
            ...
            f"💡 Ball yig'ish uchun <b>So'z o'rganish</b> bo'limidagi testni ishlang!",
            reply_markup=kb
        )
    else:
        await state.update_data(q_num=q_num + 1, correct_count=correct_count)
...
async def more_streak_cb(callback: types.CallbackQuery):
    ...
    if not user:
        ...
        return
    await show_streak(callback.message)
    await callback.answer()
...
async def grammar_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if next_index >= total:
        ...
        await callback.message.edit_text(
            ...
            f"━━━━━━━━━━━━━━━━━━\n\n"
            f"{result_msg}\n\n"
            f"💡 Ball yig'ish uchun <b>So'z o'rganish</b> bo'limini ishlating!",
            reply_markup=kb
        )
    else:
...
async def show_streak(message: types.Message):
    ...
    if streak >= 30:
        ...
        fire_color = "🔥"
        msg        = "Sen haqiqiy ingliz tili USTOZI!"
        bg_bar     = "🟥"
    elif streak >= 14:
        badge      = "💎"
        title      = "DIAMOND DARAJA"
        fire_color = "🔥"
        msg        = "Zo'r! 2 hafta uzluksiz o'rganmoqdasan!"
        bg_bar     = "🟦"
    elif streak >= 7:
        badge      = "🏆"
        title      = "CHEMPION DARAJA"
        fire_color = "🔥"
        msg        = "1 hafta uzluksiz — ajoyib iroda!"
        bg_bar     = "🟨"
    elif streak >= 3:
        badge      = "⚡️"
        title      = "ENERGIYA DARAJA"
        fire_color = "🔥"
        msg        = "Yaxshi boshladingiz! Davom eting!"
        bg_bar     = "🟩"
    elif streak >= 1:
        badge      = "🌟"
        title      = "YANGI BOSHLANISH"
        fire_color = "🔥"
        msg        = "Birinchi qadam eng muhimi — zo'rsiz!"
        bg_bar     = "🟩"
    else:
        badge      = "🌱"
        title      = "ENDI BOSHLANG"
        fire_color = "⬜"
        msg        = "Bugun birinchi kuni boshlang!"
        bg_bar     = "⬜"

    # Streak progress bar (10 ta katak)
    filled     = min(streak, 10)
    bar        = fire_color * filled + "⬜" * (10 - filled)

    # Sovrin maqsad
    prize_pct  = min(100, int(score / PRIZE_BALL_TARGET * 100))
    prize_filled = prize_pct // 10
    prize_bar  = "🟨" * prize_filled + "⬜" * (10 - prize_filled)
    prize_left = max(0, PRIZE_BALL_TARGET - score)
    if score >= PRIZE_BALL_TARGET:
        prize_line = f"✅ <b>{PRIZE_BALL_TARGET:,} ball</b> — Sovrin yuting! 🎊"
    else:
        prize_line = f"Yana <b>{prize_left:,} ball</b> → 💰 <b>{PRIZE_SCORE_AMOUNT:,} so'm</b> sovrin!"

    # Keyingi maqsad
    milestones  = [3, 7, 14, 30, 60, 100]
    next_target = next((m for m in milestones if m > streak), None)
    if next_target:
        left_days    = next_target - streak
        next_line    = f"🎯 Keyingi maqsad: <b>{next_target} kun</b> — yana <b>{left_days} kun</b>!"
    else:
        next_line    = "🎯 Barcha maqsadlarni zabt etdingiz! 🏅"

    # Streak rekord
    record_line = f"🥇 Rekord: <b>{best} kun</b>" if best > streak else (
        f"🥇 Rekord: <b>{streak} kun</b> ← bu yangi rekord!" if streak > 0 else "🥇 Hali rekord yo'q"
    )

    await message.answer(
        f"╔══════════════════╗\n"
        f"   {badge} <b>{title}</b> {badge}\n"
        f"╚══════════════════╝\n\n"
        f"🔥 <b>STREAK</b>\n"
        f"{bar}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"📅 Hozirgi: <b>{streak} kun</b>\n"
        f"{record_line}\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"💬 <i>{msg}</i>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>🏅 NISHONLAR:</b>\n"
        f"{'✅' if streak>=3  else '🔲'} 3 kun   — ⚡️ Energiya\n"
        f"{'✅' if streak>=7  else '🔲'} 7 kun   — 🏆 Chempion\n"
        f"{'✅' if streak>=14 else '🔲'} 14 kun  — 💎 Diamond\n"
        f"{'✅' if streak>=30 else '🔲'} 30 kun  — 👑 Ustoz\n"
        f"{'✅' if streak>=60 else '🔲'} 60 kun  — 🚀 Legenda\n"
        f"{'✅' if streak>=100 else '🔲'} 100 kun — 🌟 MASTER!\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"{next_line}\n\n"
        f"<b>💰 SOVRIN MAQSAD:</b>\n"
        ...
        f"⭐️ Ball: <b>{score:,}</b>\n"
        f"{prize_line}\n\n"
        f"<i>⏰ Har kun kiring — streakni uzmaslik uchun!</i>"
    )
...
async def prize_menu(message: types.Message):
    try:
        ...
        # Progress bar
        pct    = min(100, int(score / PRIZE_BALL_TARGET * 100))
        filled = pct // 10
        ...
        # Status
        if score >= PRIZE_BALL_TARGET and ref_ok:
            status = (
                "🎉 <b>TABRIKLAYMIZ! Siz barcha shartlarni bajardingiz!</b>\n"
                "Admin siz bilan tez orada bog'lanadi."
            )
        elif score >= PRIZE_BALL_TARGET and not ref_ok:
            status = (
                f"⚠️ Ball yetarli! Lekin pul yechish uchun\n"
                f"kamida <b>{PRIZE_MIN_REFERRAL} ta do'st</b> taklif qiling!\n"
                f"Hozir: <b>{ref_count} ta</b> referal"
            )
        else:
            status = f"⏳ Maqsadgacha: <b>{remaining:,} ball</b> qoldi"

        ref_status = "✅ Bajarildi" if ref_ok else f"❌ {ref_count}/{PRIZE_MIN_REFERRAL} ta"

        text = (
            "💰 <b>PUL YUTUG'I</b>\n\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "🏆 <b>DIQQAT — KATTA TANLOV!</b>\n\n"
            "🎯 Kim birinchi bo'lib <b>10,000 ball</b> yig'sa\n"
            "va kamida <b>1 do'st</b> taklif qilsa —\n"
            "unga <b>10,000 so'm</b> haqiqiy pul beramiz!\n\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "📊 <b>Sizning holatингиз:</b>\n\n"
            f"⭐️ Ball: <b>{score:,} / {PRIZE_BALL_TARGET:,}</b>\n"
            f"{bar}\n\n"
            f"👥 Referal sharti: {ref_status}\n\n"
            f"{status}\n\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "📌 <b>Shartlar:</b>\n"
            f"  ✅ <b>{PRIZE_BALL_TARGET:,} ball</b> yig'ing\n"
            f"  ✅ Kamida <b>{PRIZE_MIN_REFERRAL} ta do'st</b> taklif qiling\n"
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🏆 1-o'yin: Ball to'plash",   callback_data="prize_ball_info")],
            [InlineKeyboardButton(text="👥 2-o'yin: Referal o'yini",  callback_data="prize_ref_oyini")],
            [InlineKeyboardButton(text="👥 Do'st taklif qilish",      callback_data="prize_referral")],
        ...
        logger.error(f"prize_menu xatosi: {e}")
        await message.answer(
            "💰 <b>PUL YUTUG'I</b>\n\n"
            "10,000 ball yig'ib 10,000 so'm yuting!\n\n"
...
@dp.callback_query(F.data == "prize_referral")
async def prize_referral_cb(callback: types.CallbackQuery):
    user_id  = callback.from_user.id
    user     = get_user(user_id)
    bot_info = await bot.get_me()
    ref_link = f"https://t.me/{bot_info.username}?start={user_id}"
...
@dp.callback_query(F.data == "prize_ball_info")
async def prize_ball_info_cb(callback: types.CallbackQuery):
    user_id   = callback.from_user.id
    user      = get_user(user_id)
    score     = (user.get("score") or 0) if user else 0
    ...
    ref_ok    = ref_count >= PRIZE_MIN_REFERRAL
    if score >= PRIZE_BALL_TARGET and ref_ok:
        status = "🎉 <b>TABRIKLAYMIZ! Admin siz bilan bog'lanadi!</b>"
    elif score >= PRIZE_BALL_TARGET:
        status = f"⚠️ Ball yetarli! Kamida {PRIZE_MIN_REFERRAL} ta referal kerak!"
    else:
        status = f"⏳ Yana <b>{remaining:,} ball</b> qoldi"
    await callback.message.answer(
        f"🏆 <b>1-O'YIN: BALL TO'PLASH</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🎯 Maqsad: <b>{PRIZE_BALL_TARGET:,} ball</b> + <b>1 ta referal</b>\n"
        f"💰 Sovrin: <b>{PRIZE_SCORE_AMOUNT:,} so'm</b>\n\n"
        f"📊 Sizning holatiz:\n"
        f"⭐️ Ball: <b>{score:,}</b>\n"
        f"{bar}\n"
        f"👥 Referal: <b>{ref_count} ta</b> {'✅' if ref_ok else '❌'}\n\n"
        f"{status}\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💡 Ball yig'ish yo'llari:\n"
        f"  📚 So'z o'rganish → <b>+2 ball</b>\n"
        f"  🧠 To'g'ri test   → <b>+10 ball</b>\n"
        f"  👥 Referal        → <b>+200 ball</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="👥 Referal havolam", callback_data="prize_referral")],
            [InlineKeyboardButton(text="📚 So'z o'rganish",  callback_data="next_word")],
        ])
    )
    await callback.answer()

@dp.callback_query(F.data == "prize_ref_oyini")
async def prize_ref_oyini_cb(callback: types.CallbackQuery):
    user_id   = callback.from_user.id
    user      = get_user(user_id)
    ref_count = (user.get("referral_count") or 0) if user else 0
    ...
    if ref_count >= VIP_REF_COUNT:
        ...
        status = f"⏳ G'olib bo'lish uchun yana <b>{needed} ta do'st</b> taklif qiling."
    bot_info   = await bot.get_me()
    ref_link   = f"https://t.me/{bot_info.username}?start={user_id}"
    share_text = (
        f"Hech kim meni ingliz tili duellarida yuta olmayapti! 🧠 "
        f"Qani, biliming yetarli bo'lsa, men bilan bellashib ko'rchi? "
        f"G'olib 50 ball oladi! 👇 {ref_link}"
    )
    await callback.message.answer(
        f"👥 <b>2-O'YIN: REFERAL O'YINI</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
async def show_rating(message: types.Message):
    ...
    await message.answer(
        ...
        f"🎯 <b>{PRIZE_BALL_TARGET:,} ball → 💰 {PRIZE_SCORE_AMOUNT:,} so'm sovrin!</b>\n"
        f"━━━━━━━━━━━━━━━\n🏅 Qaysi ro'yxatni ko'rmoqchisiz?",
        reply_markup=kb
    )

def _user_profile_link(u: dict) -> str:
    """Foydalanuvchi ismi — profiliga o'tadigan havola"""
    uid      = u.get("user_id") or u.get("id")
    name     = (u.get("name") or "").strip() or f"User{uid}"
    username = (u.get("username") or "").strip()
    if username:
        url = f"https://t.me/{username}"
    else:
        url = f"tg://user?id={uid}"
    # HTML <a href> — botda inline link sifatida ishlaydi
...
async def show_top_score(callback: types.CallbackQuery):
    ...
    text  += f"🎯 <b>Maqsad: {PRIZE_BALL_TARGET:,} ball → 💰 {PRIZE_SCORE_AMOUNT:,} so'm!</b>\n"
    text  += "━━━━━━━━━━━━━━━━━━\n"
    medals = ["🥇","🥈","🥉"]
    for i, u in enumerate(top, 1):
        ...
        link    = _user_profile_link(u)
        sc      = u.get("score", 0)
        needed  = max(0, PRIZE_BALL_TARGET - sc)
        if i == 1:
            if sc >= PRIZE_BALL_TARGET:
                extra = " 🏆 <b>G'OLIB!</b>"
            else:
                extra = f" <i>({needed:,} ball qoldi)</i>"
        else:
            extra = ""
        text += f"{medal} {link}{vip} — {sc:,} ⭐️{extra}\n"
...
async def guide_section(callback: types.CallbackQuery):
    ...
    texts = {
        ...
        "referal": (
            "👥 <b>REFERAL TIZIMI</b>\n\n"
            "Do'stlaringizni taklif qilib ball va VIP yuting!\n\n"
            "<b>Qanday ishlaydi?</b>\n"
...
async def vip_ref_claim_cb(callback: types.CallbackQuery):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id,
                f"💎 <b>7 REFERAL ORQALI VIP</b>\n\n"
                f"👤 {tag} (ID: <code>{user_id}</code>)\n"
                f"📛 Ism: <b>{name}</b>\n"
                f"👥 Referallar: <b>{ref_count} ta</b>\n\n"
                f"✅ VIP avtomatik faollashtirildi."
            )
        except Exception:
            pass
...
def _admin_kb():
    bonus_label = "✅ Yoqilgan" if WEEKLY_BONUS_ENABLED else "❌ O'chirilgan"
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"🎁 Haftalik bonus: {bonus_label}", callback_data="adm_toggle_bonus")],
    ])


@dp.callback_query(F.data == "adm_toggle_bonus")
async def adm_toggle_bonus_cb(callback: types.CallbackQuery):
    global WEEKLY_BONUS_ENABLED
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    WEEKLY_BONUS_ENABLED = not WEEKLY_BONUS_ENABLED
    status = "✅ Yoqildi" if WEEKLY_BONUS_ENABLED else "❌ O'chirildi"
    await callback.answer(f"Haftalik bonus: {status}", show_alert=True)
    stats = get_stats()
    await callback.message.edit_text(
        f"👨‍💼 <b>ADMIN PANEL</b>\n\n"
        f"👥 Jami: <b>{stats['total']}</b>\n"
    ...
    while True:
        try:
            ...
            if not WEEKLY_BONUS_ENABLED:
                logger.info("Haftalik bonus o'chirilgan — admin tomonidan")
                continue
            top_list = get_top_referrals(1)
            if top_list:
                winner = top_list[0]
                if winner.get("referral_count", 0) >= int(_os.getenv("REFERRAL_MIN", 5)):
...
def _battle_id(uid1: int, uid2: int) -> str:
    a, b = sorted([uid1, uid2])
    return f"{a}_{b}"

def _get_battle_pool(user_id: int) -> list:
    """Foydalanuvchi so'z pooli: avval o'rganilganlar, keyin o'rganilmaganlar"""
    user = get_user(user_id)
    if not user:
        return words.get("beginner", [])[:60]
    learned = user.get("learned_words") or []
    level   = user.get("level", "beginner")
    all_w   = words.get(level, words.get("beginner", []))
    # Avval o'rganilganlar, keyin o'rganilmaganlar — takrorlanmaydi
    learned_pool   = [w for w in all_w if w["word"] in learned]
    unlearned_pool = [w for w in all_w if w["word"] not in learned]
    pool = learned_pool + unlearned_pool
    # Hali ham yetmasa — beginner dan to'ldirish
    if len(pool) < 40:
        bg = words.get("beginner", [])
        pool = pool + [w for w in bg if w not in pool]
    return pool

def _make_battle_question(pool: list, asked: list) -> dict | None:
    """Takrorlanmaydigan savol qaytaradi: avval o'rganilganlar, tugasa o'rganilmaganlar"""
    if not pool:
        return None
    remaining = [w for w in pool if w["word"] not in asked]
    if not remaining:
        # Hammalari so'raldi — yana pool dan boshqa so'z yo'q, barini qayta ishlatamiz
        remaining = pool
    correct = remaining[0] if remaining else random.choice(pool)  # Tartib saqlansin

    # Noto'g'ri variantlar — pool dan, yetmasa barcha so'zlardan
    wrongs = [w for w in pool if w["word"] != correct["word"]]
    if len(wrongs) < 3:
        for level_name in ("beginner", "intermediate", "advanced"):
            extra = words.get(level_name, [])
            wrongs += [w for w in extra if w["word"] != correct["word"] and w not in wrongs]
            if len(wrongs) >= 3:
                break
    if len(wrongs) < 3:
        return None
    sample = random.sample(wrongs, 3)
    opts   = [correct] + sample
    random.shuffle(opts)
    return {"correct": correct, "options": opts}

async def _send_battle_question(uid: int, battle: dict):
    """Foydalanuvchiga savol yuboradi"""
    q_num  = battle.get("q_num", 1)
    total  = BATTLE_QUESTION_COUNT
    asked1 = battle.get("asked1", [])
    asked2 = battle.get("asked2", [])

    # Har ikkala o'yinchidan o'rganilganlarni birlashtir
    pool   = battle.get("pool", [])
    asked  = asked1 if uid == battle["uid1"] else asked2
    q      = _make_battle_question(pool, asked)
    if not q:
        return

    correct = q["correct"]
    bid     = battle["battle_id"]
    q_num_cur = battle.get("q_num", 1)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=opt["translation"],
            callback_data=f"btl_ans_{bid}_{uid}_{opt['word']}_{correct['word']}"
        )]
        for opt in q["options"]
    ] + [
        [
            InlineKeyboardButton(text="💬 Chat", callback_data=f"btl_chat_open_{bid}_{uid}"),
            InlineKeyboardButton(text="🚪 Tark etish", callback_data=f"btl_leave_{bid}_{uid}_{q_num_cur}"),
        ]
    ])

    s1 = battle.get("score1", 0)
    s2 = battle.get("score2", 0)
    uid1, uid2 = battle["uid1"], battle["uid2"]
    my_score    = s1 if uid == uid1 else s2
    opp_score   = s2 if uid == uid1 else s1
    opp_uid     = uid2 if uid == uid1 else uid1

    # Raqib nomini aniqlash
    is_bot_battle = battle.get("is_bot_battle", False)
    if is_bot_battle:
        bot_profile = battle.get("bot_profile", {})
        opp_name = bot_profile.get("display_name") or "Raqib"
    else:
        opp_u = get_user(opp_uid) if opp_uid != BOT_USER_ID else None
        opp_name = (opp_u.get("name") or opp_u.get("username") or "Raqib") if opp_u else "Raqib"

    txt = (
        f"⚔️ <b>BELLASHUV</b>  {q_num}/{total}\n"
        f"🔵 Siz: <b>{my_score}</b>  🔴 {opp_name}: <b>{opp_score}</b>\n\n"
        f"⏱ 20 sekund!\n\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🇬🇧 <b>{correct['word'].upper()}</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
    ...
    if uid == -999999:
        return  # Bot ga xabar yuborilmaydi
    try:
        msg = await bot.send_message(uid, txt, reply_markup=kb)
        # Timeout task
        asyncio.create_task(_battle_timeout(bid, uid, msg.message_id, q_num))
    except Exception:
        pass

async def _battle_timeout(bid: str, uid: int, msg_id: int, q_num: int):
    """20 sekund o'tsa avtomatik keyingi savolga o'tadi"""
    await asyncio.sleep(BATTLE_TIMEOUT)
    battle = ACTIVE_BATTLES.get(bid)
    if not battle or battle.get("finished"):
        return
    if battle.get("q_num", 1) != q_num:
        return  # Already advanced

    # Xabarni tahrirlash
    try:
        await bot.edit_message_text(
            f"⏱ Vaqt tugadi! Keyingi savol...",
            chat_id=uid, message_id=msg_id
        )
    except Exception:
        pass

    # Mark this user's turn as missed
    key = "ans1_done" if uid == battle["uid1"] else "ans2_done"
    battle[key] = True

    # Bot uchun miss tracking kerak emas
    if uid != -999999:
        await _battle_track_miss(bid, uid)
        if bid not in ACTIVE_BATTLES:
            return
    await _check_both_answered(bid)

async def _check_both_answered(bid: str):
    """Ikki o'yinchi ham javob berganini tekshir"""
    battle = ACTIVE_BATTLES.get(bid)
    if not battle or battle.get("finished"):
        return

    ans1 = battle.get("ans1_done", False)
    ans2 = battle.get("ans2_done", False)

    if not (ans1 and ans2):
        return

    # Ikkalasi ham javob berdi — keyingi savol
    battle["ans1_done"] = False
    battle["ans2_done"] = False
    q_num = battle.get("q_num", 1) + 1
    battle["q_num"] = q_num

    asked_word = battle.get("last_correct")
    if asked_word:
        battle.setdefault("asked1", []).append(asked_word)
        battle.setdefault("asked2", []).append(asked_word)

    if q_num > BATTLE_QUESTION_COUNT:
        await _finish_battle(bid, reason="complete")
        return

    await _send_battle_question(battle["uid1"], battle)
    await _send_battle_question(battle["uid2"], battle)

async def _finish_battle(bid: str, reason: str = "complete", winner_id: int = None, loser_id: int = None):
    """Bellashuvni yakunlash va natijalarni e'lon qilish"""
    battle = ACTIVE_BATTLES.pop(bid, None)
    if not battle:
        return
    ...
    if reason == "complete" or reason == "leave_friend":
        if s1 > s2:
            w_id, l_id = uid1, uid2
        elif s2 > s1:
            w_id, l_id = uid2, uid1
            ...
            w_id = l_id = None  # draw
    elif reason == "leave_random":
        w_id, l_id = winner_id, loser_id
    else:
        w_id = l_id = None

    # Ball berish (bot uchun add_score chaqirilmaydi)
    _bot_id = -999999
    if reason == "leave_random":
        if w_id and w_id != _bot_id:
            add_score(w_id, BATTLE_WIN_BALL)
        # l_id ball jazosi already_penalized bo'lsa qayta berilmaydi
        if l_id and l_id != _bot_id and not battle.get("already_penalized"):
            add_score(l_id, BATTLE_LEAVE_BALL)
    elif reason == "leave_friend":
        if w_id and w_id != l_id and w_id != _bot_id:
            add_score(w_id, BATTLE_WIN_BALL)
    else:
        if w_id and w_id != l_id:
            if w_id != _bot_id:
                add_score(w_id, BATTLE_WIN_BALL)
            if l_id != _bot_id:
                add_score(l_id, BATTLE_LOSE_BALL)
        # Draw — hech kimga ball yo'q

    # ── O'yin soni va streak yangilash ──
    is_bot_battle_fin = battle.get("is_bot_battle", False)
    _bot_id_fin = -999999

    # Barcha real o'yinchilarga games++ (bir marta)
    for real_uid in [uid1, uid2]:
        if real_uid == _bot_id_fin:
            continue
        s = USER_BATTLE_STATS.setdefault(real_uid, {"wins": 0, "losses": 0, "streak": 0, "games": 0})
        s["games"] = s.get("games", 0) + 1

    # Bot bellashuvida streak ham yangilansin (games yuqorida allaqachon oshirildi)
    if is_bot_battle_fin:
        if w_id == uid1:
            _update_battle_result(uid1, won=True)
        elif l_id == uid1:
            _update_battle_result(uid1, won=False)

    # Xabar yuborish
    # Raqib ismini/havolasini olish
    def _get_player_display(uid):
        """O'yinchi ismi va havolasini qaytaradi"""
        if uid == BOT_USER_ID:
            return None, None
        u = get_user(uid)
        if not u:
            return f"Raqib", None
        name = u.get("name") or u.get("username") or f"Raqib"
        uname = u.get("username")
        link = f"https://t.me/{uname}" if uname else None
        return name, link

    async def notify(uid, opp_uid, my_s, opp_s, is_winner, is_draw, is_leave_penalty, opp_display_name=None, opp_link=None):
        if is_draw:
            emoji = "🤝"
            result = "DURRANG!"
            ball_txt = "±0 ball"
            ...
            emoji = "🏆"
            result = "G'ALABA!"
            ball_txt = f"+{BATTLE_WIN_BALL} ball"
        else:
            emoji = "😔"
            result = "MAG'LUBIYAT"
            if is_leave_penalty:
                ball_txt = f"{BATTLE_LEAVE_BALL} ball (chiqib ketish)"
            else:
                ball_txt = f"{BATTLE_LOSE_BALL} ball"
        if opp_display_name is None:
            opp_display_name, opp_link = _get_player_display(opp_uid)
        if opp_link:
            opp_tag = f'<a href="{opp_link}">{opp_display_name}</a>'
        else:
            opp_tag = f"<b>{opp_display_name}</b>"

        # Mening ma'lumotlarim (ismi + havola)
        my_u = get_user(uid)
        if my_u:
            my_name = my_u.get("name") or my_u.get("username") or "Siz"
            my_uname = my_u.get("username")
            my_link = f"https://t.me/{my_uname}" if my_uname else None
            my_tag = f'<a href="{my_link}">{my_name}</a>' if my_link else f"<b>{my_name}</b>"
        else:
            my_tag = "<b>Siz</b>"

        # Qayta o'ynash tugmasi
        rematch_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚔️ Qayta bellashish", callback_data="battle_random")],
            [InlineKeyboardButton(text="🏠 Bosh menyu", callback_data="back_menu")],
        ])
        try:
            await bot.send_message(
                uid,
                f"⚔️ <b>BELLASHUV TUGADI!</b>\n\n"
                f"{emoji} <b>{result}</b>\n\n"
                f"📊 <b>Hisobot:</b>\n"
                f"  🔵 {my_tag}: <b>{my_s}</b>\n"
                f"  🔴 {opp_tag}: <b>{opp_s}</b>\n\n"
    ...
    is_draw = (w_id is None or w_id == l_id)
    u1_wins = (w_id == uid1)
    u2_wins = (w_id == uid2)
    u1_leave_pen = (reason == "leave_random" and l_id == uid1)
    u2_leave_pen = (reason == "leave_random" and l_id == uid2)

    is_bot_battle = battle.get("is_bot_battle", False)
    bot_profile   = battle.get("bot_profile", {})

    if is_bot_battle:
        # Bot ismi — display_name (o'zbek ismi)
        bot_display_name = bot_profile.get("display_name") or bot_profile.get("name", "Raqib")
        if is_draw:
            emoji  = "🤝"; result = "DURRANG!"; ball_txt = "±0 ball"
        elif u1_wins:
            emoji  = "🏆"; result = "G'ALABA!";  ball_txt = f"+{BATTLE_WIN_BALL} ball"
        else:
            emoji  = "😔"; result = "MAG'LUBIYAT"; ball_txt = f"{BATTLE_LOSE_BALL} ball"

        # Foydalanuvchi ismi va havolasi
        my_u1 = get_user(uid1)
        if my_u1:
            my_name1 = my_u1.get("name") or my_u1.get("username") or "Siz"
            my_uname1 = my_u1.get("username")
            my_link1 = f"https://t.me/{my_uname1}" if my_uname1 else None
            my_tag1 = f'<a href="{my_link1}">{my_name1}</a>' if my_link1 else f"<b>{my_name1}</b>"
        else:
            my_tag1 = "<b>Siz</b>"
            my_name1 = "Foydalanuvchi"
            my_uname1 = None

        # Streak va keyingi daraja
        new_streak = USER_BATTLE_STATS.get(uid1, {}).get("streak", 0)
        if u1_wins and new_streak >= 1:
            next_bot = _pick_bot_opponent(uid1)
            next_acc  = int(next_bot["accuracy"] * 100)
            next_hint = next_bot.get("hint", "")
            if new_streak >= 10:
                streak_line = f"\n🔥 <b>Ketma-ket {new_streak} marta yutdingiz!</b>\n💀 Keyingi: <b>TURBO 100%</b> bot sizni kutmoqda!"
            elif new_streak >= 7:
                streak_line = f"\n🔥 <b>Seriya: {new_streak} ta!</b>\n💀 Keyingi raqib: <b>Grandmaster ({next_acc}%)</b>"
            elif new_streak >= 5:
                streak_line = f"\n🔥 <b>Seriya: {new_streak} ta!</b>\n⚡️ Keyingi: <b>Master ({next_acc}%)</b>"
            elif new_streak >= 3:
                streak_line = f"\n🔥 <b>Seriya: {new_streak} ta!</b>\n🔴 Keyingi raqib kuchliroq: <b>{next_acc}%</b>"
            else:
                streak_line = f"\n🔥 <b>Seriya: {new_streak} ta!</b> Davom eting!"
        elif not u1_wins and not is_draw:
            streak_line = f"\n💡 Seriya tushdi. Qayta boshlaysizmi?"
        else:
            streak_line = ""

        rematch_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚔️ Qayta bellashish (kuchroq)", callback_data="battle_random")],
            [InlineKeyboardButton(text="🏠 Bosh menyu", callback_data="back_menu")],
        ])
        try:
            await bot.send_message(
        ...
        # ── Kanalga natija yuborish (bot bellashuvi) ──
        if BATTLE_RESULT_CHANNEL_ID:
            try:
                if is_draw:
                    ch_emoji = "🤝"; ch_result = "DURRANG"
                elif u1_wins:
                    ch_emoji = "🏆"; ch_result = "G'ALABA"
                else:
                    ch_emoji = "😔"; ch_result = "MAG'LUBIYAT"
                # Real o'yinchi ismi (bot ko'rinmasin)
                p1_name = my_name1
                p1_link = f"https://t.me/{my_uname1}" if my_uname1 else None
                p1_tag  = f'<a href="{p1_link}">{p1_name}</a>' if p1_link else f"<b>{p1_name}</b>"
                # Bot raqib — faqat ismi, havola yo'q
                p2_tag  = f"<b>{bot_display_name}</b>"
                bot_info = await bot.get_me()
                bot_link = f"@{bot_info.username}"
                await bot.send_message(
                    BATTLE_RESULT_CHANNEL_ID,
                    f"⚔️ <b>BELLASHUV NATIJASI</b>\n\n"
                    f"{ch_emoji} {p1_tag} vs {p2_tag}\n\n"
                    f"📊 <b>Hisobot:</b>\n"
                    f"  🔵 {p1_tag}: <b>{s1}</b>\n"
                    f"  🔴 {p2_tag}: <b>{s2}</b>\n\n"
                    f"🏁 <b>{ch_result}</b>\n\n"
        ...
        await notify(uid1, uid2, s1, s2, u1_wins, is_draw, u1_leave_pen)
        await notify(uid2, uid1, s2, s1, u2_wins, is_draw, u2_leave_pen)

        # ── Kanalga natija yuborish (real vs real) ──
        if BATTLE_RESULT_CHANNEL_ID:
            try:
                def _player_tag(uid_p):
                    u = get_user(uid_p)
                    if not u:
                        return "<b>Raqib</b>", None
                    name  = u.get("name") or u.get("username") or "Raqib"
                    uname = u.get("username")
                    link  = f"https://t.me/{uname}" if uname else None
                    tag   = f'<a href="{link}">{name}</a>' if link else f"<b>{name}</b>"
                    return tag, link

                p1_tag, _ = _player_tag(uid1)
                p2_tag, _ = _player_tag(uid2)
...
# ── 2 ta javob bermasligi → bellashuv to'xtashi ──────────────────────
BATTLE_MISS_LIMIT = 3   # ketma-ket javob bermaslik chegarasi

async def _battle_track_miss(bid: str, uid: int):
    """Ketma-ket javob bermaslikni kuzatadi"""
    battle = ACTIVE_BATTLES.get(bid)
    if not battle:
        return
    key = f"miss_{uid}"
    battle[key] = battle.get(key, 0) + 1
    if battle[key] >= BATTLE_MISS_LIMIT:
        # Raqibini g'olib deb e'lon qil
        opp = battle["uid2"] if uid == battle["uid1"] else battle["uid1"]
        is_friend = battle.get("is_friend", False)
        if is_friend:
            await _finish_battle(bid, reason="leave_friend", winner_id=opp, loser_id=uid)
        else:
            await _finish_battle(bid, reason="leave_random", winner_id=opp, loser_id=uid)
        try:
            await bot.send_message(
                uid,
                "⚠️ <b>Bellashuv to'xtatildi!</b>\n\n"
                f"Ketma-ket <b>{BATTLE_MISS_LIMIT} ta savol</b>ga javob bermaganligi sababli\n"
                "bellashuv tugatildi."
            )
            await bot.send_message(
                opp,
                "🏆 <b>Raqibingiz javob bermadi!</b>\n\n"
                "Bellashuv siz foydasiga to'xtatildi."
            )
        except Exception:
            pass

# ══════════════════════════════════════════════════
# 10 ta o'yin limiti tekshiruvi
# ══════════════════════════════════════════════════
BATTLE_FREE_LIMIT = 10

async def _check_battle_limit(user_id: int):
    """10 ta bepul o'yindan keyin 1 referral talab qilinadi."""
    stats     = USER_BATTLE_STATS.get(user_id, {})
    games     = stats.get('games', 0)
    if games < BATTLE_FREE_LIMIT:
        return True, ''
    user      = get_user(user_id)
    ref_count = (user or {}).get('referral_count', 0)
    if ref_count >= 1:
        return True, ''
    try:
        bi    = await bot.get_me()
        ...
        uname = 'bot'
    ref_link = f'https://t.me/{uname}?start={user_id}'
    msg = (
        f'🔒 <b>Bepul o\'yinlar tugadi!</b>\n\n'
        f'Siz <b>{games} ta</b> bellashuv o\'yndingiz.\n\n'
        f'━━━━━━━━━━━━━━━━━━\n'
        f'📣 Davom etish uchun atigi <b>1 ta do\'stingizni</b> taklif qiling!\n\n'
        f'🔗 Sizning havolangiz:\n'
        f'<code>{ref_link}</code>\n\n'
        f'✅ Do\'stingiz havolaga bosib botga kirsa — <b>bellashuv darhol ochiladi!</b>\n\n'
        f'💡 <i>Do\'stingiz kanalga obuna bo\'lishi shart emas — faqat botga kirsa yetarli.</i>'
    )
    return False, msg
...
# ══════════════════════════════════════════════════
# MAIN MENU — "⚔️ Do'st bilan bellashish" button
# ══════════════════════════════════════════════════
@dp.message(F.text == "⚔️ Do’st bilan bellashish")
async def battle_main_menu(message: types.Message):
    user_id = message.from_user.id
    user    = get_user(user_id)
    if not user:
        await message.answer("❌ Avval /start bosing.")
    ...
    if len(learned) < 4:
        await message.answer(
            "⚠️ Bellashish uchun kamida <b>4 ta so'z</b> o'rganishingiz kerak!\n\n"
            "📚 So'z o'rgan bo'limiga o'ting."
        )
        return

    # ── 10 ta o'yin limiti ──
    allowed, block_msg = await _check_battle_limit(user_id)
    if not allowed:
        try:
            bi = await bot.get_me()
            ref_link = f'https://t.me/{bi.username}?start={user_id}'
        except Exception:
            ref_link = ''
        ...
        await message.answer(block_msg, reply_markup=kb_block)
        return

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Random odam bilan bellashish", callback_data="battle_random")],
        [InlineKeyboardButton(text="👫 Do'stim bilan bellashish",     callback_data="battle_friend_menu")],
        [InlineKeyboardButton(text="🏠 Bosh menyu",                   callback_data="back_menu")],
    ])
    await message.answer(
        "⚔️ <b>BELLASHUV BO'LIMI</b>\n\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "🎯 <b>Qoidalar:</b>\n"
        f"  📝 {BATTLE_QUESTION_COUNT} ta savol, 20 soniyadan\n"
        f"  🏆 G'olib: <b>+{BATTLE_WIN_BALL} ball</b>\n"
        f"  😔 Mag'lub: <b>{BATTLE_LOSE_BALL} ball</b>\n"
        f"  🚪 Chiqib ketish: <b>{BATTLE_LEAVE_BALL} ball</b>\n"
        f"  🤝 Durrang: <b>±0 ball</b>\n\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "👇 Bellashuv turini tanlang:",
        reply_markup=kb
    )


# ══════════════════════════════════════════════════
# 1-BO'LIM: RANDOM ODAM BILAN BELLASHISH
# ══════════════════════════════════════════════════
@dp.callback_query(F.data == "battle_random")
async def battle_random_cb(callback: types.CallbackQuery):
    user_id = callback.from_user.id

    # Agar allaqachon navbatda bo'lsa
    if user_id in BATTLE_QUEUE:
        await callback.answer("⏳ Allaqachon navbatdasiz!", show_alert=True)
        return

    # Agar aktiv bellashuvda bo'lsa
    for bid, b in ACTIVE_BATTLES.items():
        if user_id in (b["uid1"], b["uid2"]) and not b.get("finished"):
            await callback.answer("⚔️ Allaqachon bellashuvdасиз!", show_alert=True)
            return

    # Navbatda kimdir bormi?
    if BATTLE_QUEUE:
        opp_id, opp_info = next(iter(BATTLE_QUEUE.items()))
        del BATTLE_QUEUE[opp_id]

        # Bellashuvni boshlash
        bid = _battle_id(user_id, opp_id)
        pool1 = _get_battle_pool(user_id)
        pool2 = _get_battle_pool(opp_id)
        pool  = pool1 + [w for w in pool2 if w not in pool1]
        # Yetmasa barcha darajadan to'ldirish
        if len(pool) < 60:
            for lv in ("beginner", "intermediate", "advanced"):
                pool = pool + [w for w in words.get(lv, []) if w not in pool]
                if len(pool) >= 60:
                    break
        random.shuffle(pool)
        ...
        battle = {
            "battle_id": bid,
            "uid1": opp_id, "uid2": user_id,
            "score1": 0, "score2": 0,
            "q_num": 1,
            "ans1_done": False, "ans2_done": False,
            "pool": pool,
            "asked1": [], "asked2": [],
            "is_friend": False,
            "finished": False,
        ...
        try:
            await bot.edit_message_text(
                "✅ <b>Raqib topildi!</b> Bellashuv boshlanmoqda...\n\nTayyor bo'ling! ⚔️",
                chat_id=opp_info["chat_id"],
                message_id=opp_info["msg_id"]
            )
        except Exception:
            pass

        await callback.message.answer("✅ <b>Raqib topildi!</b> Bellashuv boshlanmoqda...\n\nTayyor bo'ling! ⚔️")
        await callback.answer()

        await asyncio.sleep(1)
        await _send_battle_question(opp_id, battle)
        await _send_battle_question(user_id, battle)

    else:
        # Navbatga qo'shilish
        msg = await callback.message.answer(
            "⏳ <b>Raqib qidirilmoqda...</b>\n\n"
            "Boshqa o'yinchi topilguncha kuting.\n\n"
            "❌ Bekor qilish uchun:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="❌ Bekor qilish", callback_data="battle_cancel_queue")]
            ])
        )
        BATTLE_QUEUE[user_id] = {
            "chat_id": callback.message.chat.id,
            "msg_id":  msg.message_id
        }
        await callback.answer("⏳ Navbatga qo'shildingiz!")

        # 60 sekund kutgach navbatdan chiqar
        asyncio.create_task(_queue_timeout(user_id, msg.message_id, callback.message.chat.id))


# ── Bot raqib profili ────────────────────────────────────────────────
BOT_OPPONENTS = [
    # tier 1 → streak 0 (boshlang'ich, lekin 85%)
    {"name": "🤖 Starter",    "level": "beginner",     "accuracy": 0.85, "delay": (2, 4), "tier": 1},
    # tier 2 → streak 1
    {"name": "🤖 Normal",     "level": "beginner",     "accuracy": 0.88, "delay": (2, 3), "tier": 2},
    # tier 3 → streak 2
    {"name": "🤖 O'rta",      "level": "intermediate", "accuracy": 0.91, "delay": (1, 3), "tier": 3},
    # tier 4 → streak 3-4
    {"name": "🤖 Kuchli",     "level": "intermediate", "accuracy": 0.94, "delay": (1, 2), "tier": 4},
    # tier 5 → streak 5-6
    {"name": "🤖 Pro",        "level": "advanced",     "accuracy": 0.96, "delay": (1, 2), "tier": 5},
    # tier 6 → streak 7-9
    {"name": "🤖 Master",     "level": "advanced",     "accuracy": 0.98, "delay": (1, 2), "tier": 6},
    # tier 7 → streak 10-14
    {"name": "🤖 Grandmaster","level": "advanced",     "accuracy": 0.99, "delay": (1, 1), "tier": 7},
    # tier 8 → streak 15+ (100% — hech qachon xato qilmaydi)
    {"name": "🤖 TURBO",      "level": "advanced",     "accuracy": 1.00, "delay": (1, 1), "tier": 8},
]

def _get_user_battle_streak(user_id: int) -> int:
    """Foydalanuvchining joriy yutish seriyasini qaytaradi"""
    return USER_BATTLE_STATS.get(user_id, {}).get("streak", 0)

def _update_battle_result(user_id: int, won: bool):
    """Bellashuv natijasini saqlaydi, streak yangilaydi (games allaqachon _finish_battle da oshirilgan)"""
    stats = USER_BATTLE_STATS.setdefault(user_id, {"wins": 0, "losses": 0, "streak": 0, "games": 0})
    if won:
        stats["wins"] += 1
        stats["streak"] += 1
    else:
...
def _pick_bot_opponent(user_id: int) -> dict:
    """Streak asosida bot kuchini tanlaydi — yutgan sayin kuchayib boradi"""
    streak = _get_user_battle_streak(user_id)
    ...
    if streak == 0:
        ...
        tier_range = (6, 7)
        hint = "Grandmaster 💀 (98–99%)"
    else:
        tier_range = (7, 8)
        hint = "TURBO 🔥 (100%)"

    candidates = [b for b in BOT_OPPONENTS if tier_range[0] <= b["tier"] <= tier_range[1]]
    if not candidates:
        candidates = BOT_OPPONENTS[-2:]
    chosen = random.choice(candidates)
    chosen = dict(chosen)
    chosen["hint"] = hint
    chosen["streak_used"] = streak
    return chosen

# O'zbek ismlar ro'yxati (random matchmaking uchun)
UZ_NAMES = [
    "Sardor", "Jasur", "Sherzod", "Bobur", "Ulugbek",
    "Dilshod", "Behruz", "Sanjar", "Nodir", "Otabek",
    "Zafar", "Eldor", "Kamol", "Firdavs", "Doniyor",
    "Murod", "Ravshan", "Umid", "Tohir", "Alisher",
    "Nilufar", "Malika", "Zulfiya", "Shahlo", "Feruza",
    "Sabohat", "Mohira", "Dilorom", "Nasiba", "Gulnora",
    "Maftuna", "Barno", "Lola", "Nargiza", "Hulkar",
    "Kamola", "Ozoda", "Sitora", "Shahnoza", "Yulduz",
]

# Bot ID sifatida salbiy son ishlatamiz (hech qachon real user bo'lmaydi)
BOT_USER_ID = -999999

async def _run_bot_battle(bid: str, bot_profile: dict):
    """Bot tomonidan avtomatik javob beradi (async background task)"""
    await asyncio.sleep(1.5)  # Biroz kechikish — tabiiy ko'rinsin
    accuracy  = bot_profile.get("accuracy", 0.85)
    min_d, max_d = bot_profile.get("delay", (1, 3))
    ...
    while True:
        ...
        # Barcha botlar 7-10 soniya oralig'ida javob beradi (tabiiy ko'rinsin)
        think_time = random.uniform(7, 10)
        ...
        # Bot hali javob bermaganmi?
        if battle.get("ans2_done"):
            await asyncio.sleep(0.3)
            continue

        pool    = battle.get("pool", [])
        asked2  = battle.get("asked2", [])
        q       = _make_battle_question(pool, asked2)
        if not q:
            break

        correct = q["correct"]

        # Daraja bo'yicha to'g'ri/noto'g'ri javob ehtimoli
        # accuracy=1.0 bo'lsa har doim to'g'ri, aks holda random
        if accuracy >= 1.0 or random.random() < accuracy:
            chosen = correct["word"]   # to'g'ri javob
        else:
            wrongs = [o["word"] for o in q["options"] if o["word"] != correct["word"]]
            chosen = random.choice(wrongs) if wrongs else correct["word"]

        is_correct = (chosen == correct["word"])
        if is_correct:
            battle["score2"] = battle.get("score2", 0) + 1

        # Bot o'z asked ro'yxatini yangilaydi
        lc2 = correct["word"]
        battle.setdefault("asked2", [])
        if lc2 not in battle["asked2"]:
            battle["asked2"].append(lc2)
        battle["last_correct2"] = lc2

        # Bot javob berdi deb belgilaymiz
        battle["ans2_done"] = True
        await _check_both_answered(bid)
        # _check_both_answered q_num ni oshiradi, tsikl davom etadi

async def _queue_timeout(user_id: int, msg_id: int, chat_id: int):
    await asyncio.sleep(30)
    if user_id not in BATTLE_QUEUE:
        return  # Odam topildi, bekor

    # Odam topilmadi — bot bilan boshlaylik (foydalanuvchiga aytmaymiz)
    del BATTLE_QUEUE[user_id]

    bot_profile = _pick_bot_opponent(user_id)
    bot_id      = BOT_USER_ID

    # Bot uchun tasodifiy o'zbek ismi (takrorlanmasin)
    used_names = [b.get("bot_profile", {}).get("display_name") for b in ACTIVE_BATTLES.values()]
    available_names = [n for n in UZ_NAMES if n not in used_names]
    if not available_names:
        available_names = UZ_NAMES
    bot_display_name = random.choice(available_names)
    bot_profile = dict(bot_profile)
    bot_profile["display_name"] = bot_display_name

    pool = _get_battle_pool(user_id)
    # Barcha darajadan so'zlar qo'shilsin — pool keng bo'lsin
    for lv in ("beginner", "intermediate", "advanced"):
        pool = pool + [w for w in words.get(lv, []) if w not in pool]
        if len(pool) >= 80:
            break
    ...
    await asyncio.sleep(1)
    await _send_battle_question(user_id, battle)
    # Bot background task'ini boshlash
    asyncio.create_task(_run_bot_battle(bid, bot_profile))


@dp.callback_query(F.data == "battle_cancel_queue")
async def battle_cancel_queue_cb(callback: types.CallbackQuery):
    uid = callback.from_user.id
    if uid in BATTLE_QUEUE:
        del BATTLE_QUEUE[uid]
    await callback.message.edit_text("❌ Navbatdan chiqtingiz.")
    await callback.answer()


# Battle chat: {user_id: bid}
BATTLE_CHAT_MODE: dict = {}

# Emoji reactions for bot to occasionally send
BOT_CHAT_REACTIONS = [
    "😄", "😂", "🤔", "👍", "🔥", "😎", "😅", "🙈", "💪", "😤",
    "🥳", "😬", "🤗", "👀", "😏", "😤", "🤩", "😮", "🤦", "🎯",
]
BOT_CHAT_PHRASES = [
    "Ha ha 😄", "Zo'r!", "Qiyin-a 🤔", "Bilaman 😎",
    "Voy! 😮", "Ha, ha 😂", "Yaxshi 👍", "Davom et! 💪",
    "😅 mana shunday", "🔥🔥🔥",
]


@dp.callback_query(F.data.startswith("btl_chat_open_"))
async def btl_chat_open_cb(callback: types.CallbackQuery):
    """Battle chat oyna"""
    parts = callback.data.split("_")
    uid = int(parts[-1])
    bid = "_".join(parts[3:-1])

    if callback.from_user.id != uid:
        await callback.answer("Bu sizning bellashuvingiz emas!", show_alert=True)
        return

    battle = ACTIVE_BATTLES.get(bid)
    if not battle or battle.get("finished"):
        await callback.answer("Bellashuv tugagan.", show_alert=True)
        return

    # Chat rejimiga o'tish
    BATTLE_CHAT_MODE[uid] = bid

    # Oxirgi 5 ta xabarni ko'rsat
    msgs = battle.get("chat_msgs", [])[-5:]
    if msgs:
        chat_txt = "\n".join(msgs)
    else:
        chat_txt = "<i>Hali xabar yo'q. Birinchi siz yozing!</i>"

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Xabar yuborildi — orqaga", callback_data=f"btl_chat_close_{bid}_{uid}")],
        [InlineKeyboardButton(text="🚫 Chat yopish", callback_data=f"btl_chat_close_{bid}_{uid}")],
    ])
    await callback.message.answer(
        f"💬 <b>BATTLE CHAT</b>\n\n"
        f"{chat_txt}\n\n"
        f"📝 <b>Xabaringizni yozing</b> (100 ta belgi):\n"
        f"<i>Keyingi xabaringiz raqibingizga boradi</i>",
        reply_markup=kb
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("btl_chat_close_"))
async def btl_chat_close_cb(callback: types.CallbackQuery):
    parts = callback.data.split("_")
    uid = int(parts[-1])
    BATTLE_CHAT_MODE.pop(uid, None)
    await callback.message.edit_text("🔙 Bellashuvga qaytdingiz.", reply_markup=None)
    await callback.answer("Chat yopildi")


@dp.message(F.text & ~F.text.startswith("/"))
async def battle_chat_msg_handler(message: types.Message, state: FSMContext):
    """Battle chat rejimida xabar yuborish"""
    uid = message.from_user.id
    # FSM state bo'lsa — bu handler ishlamasin
    current_state = await state.get_state()
    if current_state is not None:
        return
    bid = BATTLE_CHAT_MODE.get(uid)
    if not bid:
        return  # Chat rejimida emas, keyingi handlerga o'tsin
    ...
    text = message.text[:100]  # Max 100 belgi
    sender_u = get_user(uid)
    sender_name = sender_u.get("name") or "Raqib" if sender_u else "Raqib"

    chat_line = f"💬 <b>{sender_name}:</b> {text}"
    battle.setdefault("chat_msgs", []).append(chat_line)

    # Chat rejimidan chiq
    BATTLE_CHAT_MODE.pop(uid, None)

    # Raqibga xabar yubor
    opp_id = battle["uid2"] if uid == battle["uid1"] else battle["uid1"]
    is_bot = battle.get("is_bot_battle", False)

    if not is_bot and opp_id != BOT_USER_ID:
        try:
            await bot.send_message(
                opp_id,
                f"💬 <b>Raqibingiz yozdi:</b>\n{text}",
            )
        except Exception:
            pass
    elif is_bot:
        # Bot tasodifiy javob beradi
        import random as _rnd
        if _rnd.random() < 0.6:
            bot_reply = _rnd.choice(BOT_CHAT_PHRASES)
            bot_display = battle.get("bot_profile", {}).get("display_name", "Raqib")
            battle["chat_msgs"].append(f"💬 <b>{bot_display}:</b> {bot_reply}")
            await asyncio.sleep(_rnd.uniform(1, 3))
            try:
                await bot.send_message(uid, f"💬 <b>{bot_display}:</b> {bot_reply}")
            except Exception:
                pass

    await message.answer("✅ Xabar yuborildi!", reply_markup=main_kb(uid))


@dp.callback_query(F.data.startswith("btl_leave_") & ~F.data.startswith("btl_leave_confirm_") & ~F.data.startswith("btl_leave_cancel_"))
async def battle_leave_cb(callback: types.CallbackQuery):
    """Bellashuvni tark etish: btl_leave_{uid1}_{uid2}_{uid}_{q_num}"""
    parts = callback.data.split("_")
    # format: btl_leave_{uid1}_{uid2}_{uid}_{q_num}
    # uid1 yoki uid2 manfiy bo'lishi mumkin (bot: -999999)
    # parts: ['btl','leave', uid1, uid2, uid, q_num]  — lekin uid1/uid2 manfiy bo'lsa '-' alohida element bo'ladi
    # Shuning uchun oxiridan parse qilamiz
    if len(parts) < 6:
        await callback.answer()
        return
    q_num_at = int(parts[-1]) if parts[-1].isdigit() else 0
    uid      = int(parts[-2])
    bid      = "_".join(parts[2:-2])
    ...
    battle = ACTIVE_BATTLES.get(bid)
    if not battle or battle.get("finished"):
        await callback.answer("Bellashuv allaqachon tugagan.", show_alert=True)
        return

    # Ogohlantiruv xabari
    if q_num_at <= 10:
        penalty = BATTLE_LEAVE_BALL  # -100
        penalty_txt = f"<b>{penalty} ball</b> (10 ta savoldan oldin)"
    else:
        penalty = -50
        penalty_txt = f"<b>-50 ball</b> (10 ta savoldan keyin)"

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Ha, tark etaman", callback_data=f"btl_leave_confirm_{bid}_{uid}_{q_num_at}")],
        [InlineKeyboardButton(text="❌ Davom etaman",    callback_data=f"btl_leave_cancel_{bid}_{uid}")],
    ])
    await callback.message.answer(
        f"⚠️ <b>DIQQAT!</b>\n\n"
        f"Bellashuvni tark etsangiz {penalty_txt} qirqiladi!\n\n"
        f"Davom etasizmi yoki tark etasizmi?",
        reply_markup=kb
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("btl_leave_confirm_"))
async def battle_leave_confirm_cb(callback: types.CallbackQuery):
    """Tark etishni tasdiqladi"""
    parts = callback.data.split("_")
    # btl_leave_confirm_{uid1}_{uid2}_{uid}_{q_num}
    if len(parts) < 7:
        await callback.answer()
        return
    q_num_at = int(parts[-1]) if parts[-1].isdigit() else 0
    uid      = int(parts[-2])
    bid      = "_".join(parts[3:-2])
    ...
    battle = ACTIVE_BATTLES.get(bid)
    if not battle or battle.get("finished"):
        await callback.message.edit_text("Bellashuv allaqachon tugagan.")
        return
    ...
    if q_num_at <= 10:
        ...
        penalty = -50
    add_score(uid, penalty)
    battle["already_penalized"] = True  # _finish_battle da qayta chegilmasin

    opp_id = battle["uid2"] if uid == battle["uid1"] else battle["uid1"]
    is_friend = battle.get("is_friend", False)

    await callback.message.edit_text(
        f"🚪 <b>Bellashuvdan chiqdingiz.</b>\n\n"
        f"💸 <b>{penalty} ball</b> qirqildi.",
        reply_markup=None
    )
    # Raqibga xabar
    try:
        if opp_id != BOT_USER_ID:
            await bot.send_message(
                opp_id,
                "🏆 <b>Raqibingiz bellashuvdan chiqdi!</b>\n\nG'alaba sizniki! 🎉"
            )
    except Exception:
        pass

    if is_friend:
        await _finish_battle(bid, reason="leave_friend", winner_id=opp_id, loser_id=uid)
    else:
        await _finish_battle(bid, reason="leave_random", winner_id=opp_id, loser_id=uid)
    await callback.answer()


@dp.callback_query(F.data.startswith("btl_leave_cancel_"))
async def battle_leave_cancel_cb(callback: types.CallbackQuery):
    """Tark etishdan voz kechdi"""
    parts = callback.data.split("_")
    uid = int(parts[-1]) if parts[-1].lstrip('-').isdigit() else callback.from_user.id
    bid = "_".join(parts[3:-1])
    if callback.from_user.id != uid:
        await callback.answer()
        return
    await callback.message.edit_text("✅ Bellashuv davom etmoqda...")
    await callback.answer("Davom etasiz!")


@dp.callback_query(F.data.startswith("btl_ans_"))
async def battle_answer_cb(callback: types.CallbackQuery):
    """Bellashuv javobi: btl_ans_{bid}_{answerer_uid}_{chosen}_{correct}"""
    parts    = callback.data.split("_")
    # bid contains _ so we need to parse carefully
    # format: btl_ans_{uid1}_{uid2}_{answerer}_{chosen}_{correct}
    # Actually bid = f"{a}_{b}" where a,b are uids
    # callback_data = f"btl_ans_{bid}_{uid}_{opt['word']}_{correct['word']}"
    # parts: ['btl', 'ans', uid1, uid2, uid, chosen, correct]
    if len(parts) < 7:
        await callback.answer()
        return

    bid      = f"{parts[2]}_{parts[3]}"
    answerer = int(parts[4])
    chosen   = parts[5]
    correct  = parts[6]

    if callback.from_user.id != answerer:
        await callback.answer("Bu sizning savolingiz emas!", show_alert=True)
        return
    ...
    is_correct = (chosen == correct)
    if is_correct:
        if answerer == battle["uid1"]:
            battle["score1"] = battle.get("score1", 0) + 1
        else:
            battle["score2"] = battle.get("score2", 0) + 1
        battle[f"miss_{answerer}"] = 0  # reset miss counter
        await callback.answer("✅ To'g'ri!")
    else:
        await callback.answer(f"❌ Noto'g'ri! To'g'ri: {correct}", show_alert=True)

    battle["last_correct"] = correct

    # Mark answered
    key = "ans1_done" if answerer == battle["uid1"] else "ans2_done"
    if battle.get(key):
        return  # Already answered (timeout case)
    battle[key] = True

    # Miss tracking
    if not is_correct:
        asyncio.create_task(_battle_track_miss_soft(bid, answerer, is_correct))

    # Edit message
    try:
        result_txt = "✅ To'g'ri!" if is_correct else f"❌ Noto'g'ri (to'g'ri: {correct})"
        await callback.message.edit_text(
            f"{callback.message.text}\n\n{result_txt}\n⏳ Raqib javob kutilmoqda...",
            reply_markup=None
        )
    except Exception:
        pass
...
async def _battle_track_miss_soft(bid: str, uid: int, is_correct: bool):
    """Noto'g'ri javob miss counter emas, faqat javob bermaganlar uchun"""
    pass  # Miss tracking only for timeouts


# ══════════════════════════════════════════════════
# 2-BO'LIM: DO'ST BILAN BELLASHISH
# ══════════════════════════════════════════════════
@dp.callback_query(F.data == "battle_friend_menu")
async def battle_friend_menu_cb(callback: types.CallbackQuery):
    user_id  = callback.from_user.id
    # Faollikni yangilash
    USER_LAST_ACTIVE[user_id] = datetime.now()

    bot_info = await bot.get_me()
    token    = f"fb_{user_id}_{random.randint(1000,9999)}"
    inv_link = f"https://t.me/{bot_info.username}?start=fbattle_{user_id}_{token}"

    FRIEND_BATTLE_INVITES[token] = {
        "inviter_id": user_id,
        "accepted":   False,
        "link":       inv_link
    }
    ...
    # Agar do'stlar bo'lsa — tanlash va faolligini ko'rsat
    def _activity_label(fid: int) -> str:
        last = USER_LAST_ACTIVE.get(fid)
        if last is None:
            return "faollik noma'lum"
        delta = datetime.now() - last
        if delta.total_seconds() < 120:
            return "🟢 faol"
        else:
            return f"🕐 {last.strftime('%H:%M')} oxirgi faollik"

    for fid in list(friends)[:8]:
        fu = get_user(fid)
        if fu:
            fname = fu.get("name") or f"User{fid}"
            act   = _activity_label(fid)
            kb_rows.append([InlineKeyboardButton(
                text=f"⚔️ {fname} ({act})",
    ...
    await callback.message.edit_text(
        "👫 <b>DO'ST BILAN BELLASHISH</b>\n\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "Do'stingizga havola yuboring yoki quyida tanglang:\n\n"
        f"🔗 <code>{inv_link}</code>\n\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "👇 Do'stingizni tanlang yoki havolani ulashing:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows)
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("battle_invite_"))
async def battle_invite_direct_cb(callback: types.CallbackQuery):
    """Do'stni to'g'ridan-to'g'ri bellashuvga taklif qilish"""
    parts  = callback.data.split("_")
    opp_id = int(parts[2])
    token  = parts[3] if len(parts) > 3 else None

    inviter_id = callback.from_user.id
    if opp_id == inviter_id:
        await callback.answer("O'zingizni taklif qila olmaysiz!", show_alert=True)
        return

    # Taklif yuborilgan vaqtni saqlash (2 daqiqa timeout uchun)
    FRIEND_BATTLE_INVITE_TIME[(inviter_id, opp_id)] = datetime.now()

    # Taklif xabarini yuborish
    try:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Qabul qilish",  callback_data=f"fbaccept_{inviter_id}_{opp_id}")],
            [InlineKeyboardButton(text="❌ Rad qilish",   callback_data=f"fbdecline_{inviter_id}")],
        ])
        inviter_u = get_user(inviter_id)
        iname     = inviter_u.get("name") if inviter_u else "Kimdir"
        msg = await bot.send_message(
            ...
            f"👤 <b>{iname}</b> sizni bellashuvga taklif qilyapti!\n\n"
            f"🎯 {BATTLE_QUESTION_COUNT} ta savol • 20 soniya • {BATTLE_WIN_BALL} ball\n\n"
            f"⏳ <b>2 daqiqa</b> ichida javob bering!\n\n"
            f"Qabul qilasizmi?",
            reply_markup=kb
        )
        await callback.answer("✅ Taklif yuborildi!")
        # 2 daqiqa timeout — javob berilmasa bekor qilish
        asyncio.create_task(_friend_invite_timeout(inviter_id, opp_id, msg.message_id))
    except Exception:
        await callback.answer("❌ Xabar yuborib bo'lmadi", show_alert=True)


async def _friend_invite_timeout(inviter_id: int, opp_id: int, msg_id: int):
    """2 daqiqa ichida javob berilmasa so'rovni bekor qilish va adminlarni xabardor qilish"""
    await asyncio.sleep(120)  # 2 daqiqa

    # Taklif hali ham kutilayotganmi?
    invite_time = FRIEND_BATTLE_INVITE_TIME.get((inviter_id, opp_id))
    if invite_time is None:
        return  # Allaqachon qabul qilindi yoki rad etildi
    # Vaqt o'tgan — bekor qilish
    FRIEND_BATTLE_INVITE_TIME.pop((inviter_id, opp_id), None)

    # Raqibga xabar — taklif muddati o'tdi
    try:
        await bot.edit_message_text(
            "⏱ <b>Bellashuv taklifi muddati o'tdi.</b>\n\n"
            "Taklif 2 daqiqa ichida qabul qilinmadi.",
            chat_id=opp_id,
            message_id=msg_id,
            reply_markup=None
        )
    ...
    inviter_u = get_user(inviter_id)
    opp_u     = get_user(opp_id)
    opp_name  = (opp_u.get("name") if opp_u else None) or f"ID:{opp_id}"
    try:
        await bot.send_message(
            inviter_id,
            f"⏱ <b>{opp_name}</b> 2 daqiqa ichida javob bermadi.\n\n"
    ...
    inviter_name = (inviter_u.get("name") if inviter_u else None) or f"ID:{inviter_id}"
    inviter_un   = inviter_u.get("username") if inviter_u else None
    opp_un       = opp_u.get("username") if opp_u else None
    inviter_tag  = f"@{inviter_un}" if inviter_un else f"ID:<code>{inviter_id}</code>"
    opp_tag      = f"@{opp_un}"    if opp_un     else f"ID:<code>{opp_id}</code>"

    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id,
                f"⏱ <b>Bellashuv taklifi bekor qilindi</b>\n\n"
                f"📤 Taklif qildi: <b>{inviter_name}</b> ({inviter_tag})\n"
                f"📥 Qabul qilmadi: <b>{opp_name}</b> ({opp_tag})\n\n"
                f"⏳ 2 daqiqa ichida javob berilmadi."
            )
        except Exception:
            pass


@dp.callback_query(F.data.startswith("fbaccept_"))
async def fb_accept_cb(callback: types.CallbackQuery):
    """Do'st bellashuvini qabul qildi"""
    parts      = callback.data.split("_")
    ...
    # Do'stlar ro'yxatiga qo'shish
    FRIEND_LIST.setdefault(inviter_id, set()).add(opp_id)
    FRIEND_LIST.setdefault(opp_id, set()).add(inviter_id)

    # Taklif qiluvchi ham roziligi so'ralsin
    FRIEND_BATTLE_INVITES[f"pending_{inviter_id}_{opp_id}"] = {
        "inviter_id": inviter_id, "opp_id": opp_id, "accepted": False
    }
    await callback.message.edit_text(
        "✅ <b>Qabul qildingiz!</b> Raqib tasdiqlashini kuting...",
        reply_markup=None
    )
    try:
        kb2 = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Boshlaymiz!", callback_data=f"fbstart_{inviter_id}_{opp_id}")],
            [InlineKeyboardButton(text="❌ Bekor qilish", callback_data=f"fbcancel_{inviter_id}_{opp_id}")],
        ])
        opp_u = get_user(opp_id)
        oname = opp_u.get("name") if opp_u else "Do'stingiz"
        await bot.send_message(
            inviter_id,
            f"✅ <b>{oname}</b> bellashuvga rozi bo'ldi!\n\n⚔️ Bellashuvni boshlaysizmi?",
            reply_markup=kb2
        )
    except Exception:
        pass
...
@dp.callback_query(F.data.startswith("fbstart_"))
async def fb_start_cb(callback: types.CallbackQuery):
    """Taklif qiluvchi bellashuvni boshlaydi"""
    parts = callback.data.split("_")
    inviter_id = int(parts[1])
    opp_id     = int(parts[2])
    user_id    = callback.from_user.id
    ...
    bid  = _battle_id(inviter_id, opp_id)
    pool1 = _get_battle_pool(inviter_id)
    pool2 = _get_battle_pool(opp_id)

    # Ikki o'yinchi o'rganilgan so'zlarini birlashtir
    pool = pool1 + [w for w in pool2 if w not in pool1]

    # Pool kichik bo'lsa — ikkalasining daraja so'zlaridan to'ldirish
    u1 = get_user(inviter_id)
    u2 = get_user(opp_id)
    lv1 = (u1 or {}).get("level", "beginner")
    lv2 = (u2 or {}).get("level", "beginner")
    extra = words.get(lv1, []) + [w for w in words.get(lv2, []) if w not in words.get(lv1, [])]
    pool = pool + [w for w in extra if w not in pool]

    # Minimum 60 ta so'z bo'lsin — yetmasa beginner dan qo'shish
    if len(pool) < 60:
        bg = words.get("beginner", [])
        pool = pool + [w for w in bg if w not in pool]
    ...
    await callback.message.edit_text(
        "✅ <b>Bellashuv boshlanmoqda...</b> ⚔️",
        reply_markup=None
    )
    try:
        await bot.send_message(opp_id, "⚔️ <b>Bellashuv boshlandi!</b> Savollar kelmoqda...")
    except Exception:
        pass

    await asyncio.sleep(1)
    await _send_battle_question(inviter_id, battle)
    await _send_battle_question(opp_id, battle)
    await callback.answer("✅ Bellashuv boshlandi!")


@dp.callback_query(F.data.startswith("fbcancel_"))
async def fb_cancel_cb(callback: types.CallbackQuery):
    """Bellashuvni bekor qildi"""
    parts = callback.data.split("_")
    inviter_id = int(parts[1])
    opp_id     = int(parts[2])
    await callback.message.edit_text("❌ Bellashuv bekor qilindi.", reply_markup=None)
    try:
        await bot.send_message(opp_id, "❌ Raqib bellashuvdan voz kechdi.")
    except Exception:
        pass
    await callback.answer()


@dp.callback_query(F.data.startswith("fbdecline_"))
async def fb_decline_cb(callback: types.CallbackQuery):
    inviter_id = int(callback.data.split("_")[1])
    decliner_id = callback.from_user.id
    # Timeout bekor qilish
    FRIEND_BATTLE_INVITE_TIME.pop((inviter_id, decliner_id), None)
    await callback.message.edit_text("❌ Bellashuv rad etildi.", reply_markup=None)
    try:
        decliner_u = get_user(callback.from_user.id)
        dname = decliner_u.get("name") if decliner_u else "Do'stingiz"
        await bot.send_message(inviter_id, f"😔 <b>{dname}</b> bellashuvni rad qildi.")
    except Exception:
        pass
    await callback.answer("Rad etildi")


# Do'st taklif havola orqali /start?start=fbattle_uid_token
# Bu /start handler ichida tekshiriladi — quyida qo'shamiz:
@dp.callback_query(F.data == "battle_invite_link")
async def battle_invite_link_cb(callback: types.CallbackQuery):
    """Referal bo'limidan do'st bellashuv havolasi"""
    user_id  = callback.from_user.id
    bot_info = await bot.get_me()
    ...
    await callback.message.answer(
        ...
        f"🔗 Sizning duel havolangiz:\n<code>{inv_link}</code>\n\n"
        f"📢 Do'stingiz havolani bosishi bilan:\n"
        f"  ⚔️ Duel avtomatik boshlanadi\n"
        f"  👥 U sizga <b>referal</b> bo'lib qo'shiladi!\n\n"
        f"👇 <b>Ulashish tugmasini bosing:</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text="🚀 Do'stlarimga yuborish",
                url=share_url_b
...
# ══════════════════════════════════════════════════
# ADMIN: /resetballs — Hamma ball 0 ga tushadi
# ══════════════════════════════════════════════════
@dp.message(Command("resetballs"))
async def cmd_reset_balls(message: types.Message):
    """Admin barcha foydalanuvchilar balini 0 ga tushiradi"""
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Ruxsat yo'q")
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ HA, 0 GA TUSHIR", callback_data="confirm_resetballs")],
        [InlineKeyboardButton(text="❌ Bekor qilish",    callback_data="cancel_resetballs")],
    ])
    await message.answer(
        "⚠️ <b>DIQQAT!</b>\n\n"
        "Bu amal <b>BARCHA foydalanuvchilar</b> balini\n"
        "<b>0 ga</b> tushiradi.\n\n"
...
@dp.callback_query(F.data == "confirm_resetballs")
async def confirm_reset_balls_cb(callback: types.CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Ruxsat yo'q", show_alert=True)
        return
    # DB da hamma score = 0
    try:
        conn = get_conn()
        cur  = conn.cursor()
        cur.execute("UPDATE users SET score = 0, prize_claimed = FALSE")
        rows = cur.rowcount
        conn.commit()
        cur.close()
        conn.close()
        ...
        await callback.message.edit_text(
            f"✅ <b>Bajarildi!</b>\n\n"
            f"👥 {rows} ta foydalanuvchi bali <b>0 ga</b> tushirildi.\n"
        ...
        for uid in user_ids:
            try:
                await bot.send_message(
                    ...
                    "⚙️ Barcha ballar 0 ga tushirildi.\n"
                    f"🎯 Maqsad: <b>{PRIZE_BALL_TARGET:,} ball</b>\n"
                    f"💰 Sovrin: <b>{PRIZE_SCORE_AMOUNT:,} so'm</b>\n\n"
                    "O'rganishni davom eting va birinchi bo'ling! 🚀"
                )
                sent += 1
                await asyncio.sleep(0.05)
            except Exception:
                pass
        await callback.message.answer(f"📢 {sent} ta userlarga xabar yuborildi.")
    except Exception as e:
        await callback.message.edit_text(f"❌ Xato: {e}")
    await callback.answer()

@dp.callback_query(F.data == "cancel_resetballs")
async def cancel_reset_balls_cb(callback: types.CallbackQuery):
...
async def cmd_restart(message: types.Message, state: FSMContext):
    """Har kim uchun — FSM holatini tozalash va menyuга qaytish"""
    user_id = message.from_user.id
    await state.clear()
    # Bellashuv navbatidan chiqarish (agar bo'lsa)
    if user_id in BATTLE_QUEUE:
        del BATTLE_QUEUE[user_id]
    # Battle chat rejimidan chiqarish
    BATTLE_CHAT_MODE.pop(user_id, None)
    await message.answer(
        "🔄 <b>Bot qayta ishga tushirildi!</b>\n\n"
        "Barcha holatlar tozalandi. Bosh menyudasiz.",
        reply_markup=main_kb(user_id)
    )
...
async def main():
    ...
    private_commands = [
        ...
        BotCommand(command="restart",   description="🔄 Botni qayta ishga tushirish"),
