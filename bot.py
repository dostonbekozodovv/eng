...
# ── Sozlamalar ──
PRIZE_SCORE_AMOUNT = 10_000   # 10 000 ball yig'gan foydalanuvchi uchun sovrin (so'm)
...
async def _check_score_prize(user_id: int):
    """10 000 ball to'plagan foydalanuvchini tekshirish"""
    user  = get_user(user_id)
    score = user.get("score", 0) if user else 0
    if score >= 10_000 and not user.get("prize_claimed"):
        try:
            ...
            for admin_id in ADMIN_IDS:
                ...
                await bot.send_message(
                    ...
                    f"⭐️ Ball: {score}\n\n"
                    f"Sovrin: {PRIZE_SCORE_AMOUNT:,} so'm to'lang!"
                )
        except Exception:
            pass
...
def main_kb(user_id: int = None, chat=None):
    ...
    buttons = [
        ...
        [KeyboardButton(text="📖 Grammatika"),    KeyboardButton(text="🏆 Reyting")],
        [KeyboardButton(text="⚙️ Daraja"),         KeyboardButton(text="📋 Qo‘llanma")],
        [KeyboardButton(text="☰ Boshqalar"),       vip_btn],
    ]
...
def more_kb():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👥 Referal",       callback_data="more_referral"),
            ...
            try:
                ...
                if referrer:
                    ...
                    if ref_total >= 3 and not is_vip(int(ref_id)):
                        import datetime
                        vip_until = (datetime.datetime.now() + datetime.timedelta(days=30)).strftime("%Y-%m-%d")
                        ...
                        try:
                            await bot.send_message(
                                ...
                                "✅ Akkauntingiz avtomatik tarzda <b>VIP statusiga</b> o'tdi!\n\n"
...
async def more_feedback_cb(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "💬 <b>TAKLIF VA FIKR</b>\n\n"
        "Botni yaxshilash uchun fikringizni yozing!\n\n"
        "• Muammo haqida\n"
...
async def guide_section(callback: types.CallbackQuery):
    ...
    texts = {
        ...
        "rating": (
            ...
            f"<b>🏆 KATTA SOVRIN:</b>\n"
            f"• <b>10 000 ball</b> to'playdi → <b>{PRIZE_SCORE_AMOUNT:,} so'm</b> yutadi!\n\n"
...
# ══════════════════════════════════════════════════
# VIP REFERAL KAMPANIYASI BROADCAST
# ══════════════════════════════════════════════════
