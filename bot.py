...
async def poll_group_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    GROUP_QUIZ_STATE[chat_id] = {
        ...
        "total":        total,
        "asked":        [],
        "scores":       {},      # { user_id: to'g'ri javoblar soni }
        "poll_answers": {},      # { poll_id: correct_option_id }
        "active":       True
    }
...
async def _send_group_poll(chat_id: int):
        ...
        if scores:
            ...
            for i, (uid, count) in enumerate(sorted_scores[:10]):
                ...
                try:
                    ...
                    name   = member.user.first_name or f"User"
                except Exception:
                    name = f"Ishtirokchi {i+1}"
                result_text += f"{medal} <b>{name}</b> — {count}/{total} ✅\n"
    ...
    # Poll ID ga to'g'ri indeksni saqlash
    qs["poll_answers"][sent.poll.id] = correct_idx

    # 32 soniyadan keyin keyingi savolni yuborish (30 sek poll + 2 sek pauza)
    asyncio.create_task(_delayed_next_poll(chat_id, 32))

async def _delayed_next_poll(chat_id: int, delay: int):
...
async def on_poll_answer(poll_answer: types.PollAnswer):
    ...
    # Qaysi chat'ga tegishli ekanligini topish
