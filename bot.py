...
@dp.callback_query(F.data.startswith("btl_leave_"))
async def battle_leave_cb(callback: types.CallbackQuery):
    """Bellashuvni tark etish: btl_leave_{bid_part1}_{bid_part2}_{uid}_{q_num}"""
    parts = callback.data.split("_")
    ...
    uid      = int(parts[4])
    q_num_at = int(parts[5]) if parts[5].isdigit() else 0
...
async def battle_leave_confirm_cb(callback: types.CallbackQuery):
    ...
    if len(parts) < 7:
        await callback.answer()
        return
    bid      = f"{parts[3]}_{parts[4]}"
    uid      = int(parts[5])
    q_num_at = int(parts[6]) if parts[6].isdigit() else 0
...
async def battle_leave_cancel_cb(callback: types.CallbackQuery):
    ...
    parts = callback.data.split("_")
    bid = f"{parts[3]}_{parts[4]}"
    uid = int(parts[5]) if len(parts) > 5 else callback.from_user.id
    if callback.from_user.id != uid:
        await callback.answer()
        return
