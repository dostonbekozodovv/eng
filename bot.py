...
async def _send_battle_question(uid: int, battle: dict):
    ...
    ] + [[InlineKeyboardButton(text="🚪 Testni tark etish", callback_data=f"btl_leave_{bid}_{uid}_{q_num_cur}")]])
...
# ── Bot raqib profili ────────────────────────────────────────────────
BOT_OPPONENTS = [
    {"name": "🤖 AlexBot",    "level": "beginner",     "accuracy": 0.55, "delay": (3, 7)},
    {"name": "🤖 SaraBot",    "level": "intermediate",  "accuracy": 0.65, "delay": (2, 6)},
    {"name": "🤖 MaxBot",     "level": "intermediate",  "accuracy": 0.70, "delay": (2, 5)},
    {"name": "🤖 NinaBot",    "level": "advanced",      "accuracy": 0.80, "delay": (1, 4)},
    {"name": "🤖 TurboBot",   "level": "advanced",      "accuracy": 0.90, "delay": (1, 3)},
]
...
async def fb_accept_cb(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        "✅ <b>Qabul qildingiz!</b> Bellashuv boshlanmoqda...",
        reply_markup=None
    )
    try:
        await bot.send_message(
            inviter_id,
            "✅ <b>Do'stingiz qabul qildi!</b> Bellashuv boshlanmoqda... ⚔️"
        )
    except Exception:
        pass
