...
async def brain_menu(message: types.Message, state: FSMContext):
    ...
    if st["n"] >= 2:
        await message.answer(
            ...
            f"💡 Javob: <tg-spoiler>{answer}</tg-spoiler>\n"
            "Ertaga yangi savol 💪"
        )
        return
...
async def brain_answer(message: types.Message, state: FSMContext):
    ...
    if st["ok"] or st["n"] >= 2:
        ...
        return
    answer = q["a"]
    correct = (answer.strip().lower() == user_ans)
...
async def brain_cancel_cb(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text("❌ Bekor qilindi.")
    await callback.answer()


@dp.message(F.text == "savol qo'sh")
...
async def brain_adm_save(message: types.Message, state: FSMContext):
    ...
    if len(parts) < 2 or not parts[0].strip() or not parts[1].strip():
        await message.answer("❌ Format xato! Savol va javobni yangi qatorda yozing.")
        return
...
@dp.message(F.text == "savol ro'yxat")
...
@dp.message(F.text.startswith("savol o'chir"))
