...
async def cmd_stopquiz(message: types.Message):
    ...
    if not qs or not qs.get("active"):
        ...
        return
    # Faqat admin yoki guruh admin to'xtatishi mumkin
    try:
        member = await bot.get_chat_member(chat_id, message.from_user.id)
        if member.status not in ("administrator", "creator") and chat_id != message.from_user.id:
            await message.reply("❌ Faqat admin to'xtatishi mumkin.")
            return
    except Exception:
        pass
...
async def poll_group_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Guruh quiz holatini module-level dict'da saqlash
    GROUP_QUIZ_STATE[chat_id] = {
        ...
        "poll_answers":      {},
        "active":            True,
        "unanswered_streak": 0,   # Ketma-ket javob berilmagan polllar soni
        "last_poll_id":      None,
    }
...
async def _send_group_poll(chat_id: int):
    ...
    # 2 ta ketma-ket javobsiz poll → avtomatik to'xtatish
    if unanswered_streak >= 2 and poll_num > 0:
        qs["active"] = False
        scores = qs.get("scores", {})
        result_text = (
    ...
    sent = await bot.send_poll(
        ...
        is_anonymous=False,
        open_period=20 if chat_id < 0 else 30
    )
    ...
    # Guruhda 22 soniya (20 sek poll + 2 sek pauza), shaxsiyda 3 soniya
    delay = 3 if chat_id > 0 else 22
...
async def _delayed_next_poll(chat_id: int, poll_id: str, delay: int):
    """Kechikish bilan keyingi poll yuborish — javob berilganini tekshirish"""
    await asyncio.sleep(delay)
    qs = GROUP_QUIZ_STATE.get(chat_id)
    if not qs or not qs.get("active"):
        return

    # Shu poll'ga hech kim javob bermaganmi tekshirish
...
async def on_poll_answer(poll_answer: types.PollAnswer):
    """Poll javoblarini kuzatish — to'g'ri javoblarni hisoblab borish"""
    poll_id  = poll_answer.poll_id
    user_id  = str(poll_answer.user.id)

    for chat_id, qs in GROUP_QUIZ_STATE.items():
        if poll_id in qs.get("poll_answers", {}):
            correct_idx = qs["poll_answers"][poll_id]
            # Javob berganlarni ro'yxatlash
...
async def set_level(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not is_group(callback.message):
        ...
        # Guruhda daraja saqlangandan keyin to'g'ridan quiz menyusini ko'rsat
        level     = callback.data.split("_", 1)[1]
        all_words = words.get(level, [])
...
async def main():
    ...
    group_commands = [
        ...
        BotCommand(command="quiz",     description="🎯 So'z viktorinasi (Poll)"),
        BotCommand(command="stopquiz", description="⏹ Viktorinani to'xtatish (admin)"),
