...
# ══════════════════════════════════════════════════
@dp.message(F.text == "💰 Pul yutug'i")
async def pul_yutugi_menu(message: types.Message):
    user_id = message.from_user.id
    user    = get_user(user_id)
    if not user:
        await message.answer("❌ Avval /start bosing.")
        ...
        try:
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(
                    ...
                    callback_data="survey_start_0"
                )]
            ])
            await bot.send_message(
                uid,
                "📋 <b>MAXSUS SO'ROVNOMA</b>\n\n"
                "Botni yaxshilash uchun sizning fikringiz muhim!\n"
                f"Jami <b>{len(SURVEY_QUESTIONS)} ta savol</b> — 1 daqiqa vaqt oladi.\n\n"
