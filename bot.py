        ...
        if ref_id and not user.get("referral_notified"):
            try:
                ...
                await bot.send_message(
                    int(ref_id),
                    f"🎉 <b>Yangi referal!</b>\n\n"
                    f"✅ Siz taklif qilgan <b>{name}</b> kanalga obuna bo'ldi!\n"
                    f"⭐️ <b>+{REFERRAL_BONUS} ball</b> qo'shildi!"
                )
...
async def _battle_timeout(bid: str, uid: int, msg_id: int, q_num: int):
    ...
    # Track missed answer for auto-finish if too many misses
    await _battle_track_miss(bid, uid)
    if bid not in ACTIVE_BATTLES:
        return  # Battle finished by miss limit
...
async def _check_both_answered(bid: str):
    ...
    asked1_word = battle.get("last_correct1")
    asked2_word = battle.get("last_correct2")
    if asked1_word:
        battle.setdefault("asked1", []).append(asked1_word)
    if asked2_word:
        battle.setdefault("asked2", []).append(asked2_word)
...
async def _queue_timeout(user_id: int, msg_id: int, chat_id: int):
    ...
    if user_id in BATTLE_QUEUE:
        ...
        try:
            await bot.edit_message_text(
                "😔 Raqib topilmadi. Qayta urinib ko'ring.",
...
async def battle_answer_cb(callback: types.CallbackQuery):
    ...
    # Track per-player correct word to avoid repeats
    lc_key = "last_correct1" if answerer == battle["uid1"] else "last_correct2"
    battle[lc_key] = correct
