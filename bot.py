...
async def grammar_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if next_index >= total:
        ...
        await callback.message.edit_text(
            ...
            f"━━━━━━━━━━━━━━━━━━\n\n"
          # O'zgaruvchiga biriktirishda oddiy qo'shtirnoq ishlatiladi, shuning uchun ichida ' (bittalik tirnoq) bemalol ishlayveradi, \ shart emas:
xabar = "🔥 Zo'r! Siz grammatikani bilasiz!" if percent >= 80 else "💪 Tushuntirishni qayta o'qing va urinib ko'ring!"

# Keyin uni f-string ichida chaqirib qo'yasiz:
            f"{xabar}"
            reply_markup=kb
        )
    else:
...
async def referal_menu(message: types.Message):
    ...
    await message.answer(
        ...
        f"{progress_bar}\n"
        f"{'🏆 G\'olib bo\'lishingiz mumkin!' if ref_count>=5 else f'📌 G\'olib uchun yana {needed} ta kerak'}\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
