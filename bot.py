...
def _get_battle_pool(user_id: int) -> list:
    """Foydalanuvchi o'rganilgan so'zlaridan test pool oladi, yetmasa daraja so'zlari qo'shiladi"""
    user = get_user(user_id)
    if not user:
    ...
    pool    = [w for w in all_w if w["word"] in learned]
    # O'rganilmagan so'zlar ham qo'shilsin (pool kengaysin)
    if len(pool) < 40:
        unlearned = [w for w in all_w if w["word"] not in learned]
        pool = pool + unlearned
...
def _make_battle_question(pool: list, asked: list) -> dict | None:
    """Takrorlanmaydigan savol qaytaradi"""
    if not pool:
        return None
    remaining = [w for w in pool if w["word"] not in asked]
    if not remaining:
        remaining = pool  # Hammalari so'raldi — qaytadan boshla
    correct = random.choice(remaining)
...
async def _send_battle_question(uid: int, battle: dict):
    ...
    # Bot bellashuvida 1-savolda tier ko'rsat
    if is_bot_battle and q_num == 1:
        bot_profile = battle.get("bot_profile", {})
        acc_pct = int(bot_profile.get("accuracy", 0.85) * 100)
        hint    = bot_profile.get("hint", "")
        streak  = bot_profile.get("streak_used", 0)
        if streak >= 1:
            tier_line = f"\n🎯 Raqib kuchi: <b>{hint}</b> ({acc_pct}%)"
        else:
            tier_line = ""
        txt += tier_line
    if uid == -999999:
...
async def _run_bot_battle(bid: str, bot_profile: dict):
    ...
    while True:
        ...
        # 100% botda juda tez javob beradi (0.5-1 sekund)
        if accuracy >= 1.0:
            think_time = random.uniform(0.4, 0.9)
        else:
            think_time = random.uniform(min_d, max_d)
...
async def _queue_timeout(user_id: int, msg_id: int, chat_id: int):
    ...
    # Streak va bot kuchi haqida xabar
    streak = bot_profile.get("streak_used", 0)
    hint   = bot_profile.get("hint", "")
    acc_pct = int(bot_profile["accuracy"] * 100)
    if streak >= 1:
        streak_msg = (
            f"🔥 <b>Yutish seriyangiz: {streak} ta!</b>\n\n"
            f"⚠️ Raqibingiz kuchliroq: <b>{hint}</b>\n"
            f"🎯 Bot aniqligi: <b>{acc_pct}%</b>\n\n"
            f"Kuchingizni sinang! 💪"
        )
        try:
            await bot.send_message(user_id, streak_msg)
        except Exception:
            pass
