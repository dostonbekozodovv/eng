...
LEVEL_TEST_BONUS   = {"beginner": 10, "intermediate": 20, "advanced": 30}
...
async def _send_next_word(target, user_id: int, edit: bool = False):
    ...
    word_ball  = LEVEL_TEST_BONUS.get(level, WORD_LEARN_BONUS)  # beginner=10,inter=20,adv=30 ... wait
    # So'z o'rganish uchun: beginner=2, intermediate=2, advanced=2 (2 ball — daraja farq qilmaydi)
    # Faqat birinchi marta — add_learned_word chaqirilganida unlearned listdan chiqadi
    ...
    txt = (
        ...
        f"⭐️ +{WORD_LEARN_BONUS} ball qo'shildi!"
    )
    if edit:
        await target.edit_text(txt, reply_markup=kb)
    else:
...
def _get_battle_pool(user_id: int) -> list:
    """Foydalanuvchi o'rganilgan so'zlaridan test pool oladi"""
    user = get_user(user_id)
    if not user:
        return []
    learned = user.get("learned_words") or []
...
def _make_battle_question(pool: list, asked: list) -> dict | None:
    ...
    if len(wrongs) < 3:
        all_words_flat = words.get("beginner", [])
        wrongs = [w for w in all_words_flat if w["word"] != correct["word"]]
    sample  = random.sample(wrongs, min(3, len(wrongs)))
    opts    = [correct] + sample
    random.shuffle(opts)
    ...
    async def notify(uid, opp_uid, my_s, opp_s, is_winner, is_draw, is_leave_penalty, opp_display_name=None, opp_link=None):
        ...
        try:
            await bot.send_message(
                ...
                f"  🔴 {opp_tag}: <b>{opp_s}</b>\n\n"
                f"💰 {ball_txt}\n\n"
                f"🔙 Qayta o'ynash: ⚔️ tugmasini bosing",
                disable_web_page_preview=True
            )
        except Exception:
            pass
...
# ── Bot raqib profili ────────────────────────────────────────────────
BOT_OPPONENTS = [
    {"name": "🤖 AlexBot",    "level": "beginner",     "accuracy": 0.72, "delay": (2, 5)},
    {"name": "🤖 SaraBot",    "level": "intermediate",  "accuracy": 0.78, "delay": (2, 4)},
    {"name": "🤖 MaxBot",     "level": "intermediate",  "accuracy": 0.83, "delay": (1, 4)},
    {"name": "🤖 NinaBot",    "level": "advanced",      "accuracy": 0.90, "delay": (1, 3)},
    {"name": "🤖 TurboBot",   "level": "advanced",      "accuracy": 0.97, "delay": (1, 2)},
]

def _pick_bot_opponent(user_id: int) -> dict:
    """Foydalanuvchi darajasiga qarab mos bot tanlaydi"""
    user  = get_user(user_id)
    level = (user or {}).get("level", "beginner")
    if level == "beginner":
        candidates = [BOT_OPPONENTS[0], BOT_OPPONENTS[1]]
    elif level == "intermediate":
        candidates = [BOT_OPPONENTS[1], BOT_OPPONENTS[2], BOT_OPPONENTS[3]]
    else:
        candidates = [BOT_OPPONENTS[2], BOT_OPPONENTS[3], BOT_OPPONENTS[4]]
...
async def _run_bot_battle(bid: str, bot_profile: dict):
    ...
    while True:
        ...
        # Bot uchun javob vaqtini simulatsiya qilish
        min_d, max_d = bot_profile["delay"]
        ...
        if random.random() < bot_profile["accuracy"]:
...
async def _queue_timeout(user_id: int, msg_id: int, chat_id: int):
    ...
    # Bot uchun ham so'zlar pool'iga daraja so'zlari qo'shiladi
    bot_level_words = words.get(bot_profile["level"], words.get("beginner", []))[:40]
    pool = pool + [w for w in bot_level_words if w not in pool]
    if len(pool) < 4:
        pool = words.get("beginner", [])[:40]
    random.shuffle(pool)
