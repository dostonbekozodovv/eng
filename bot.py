...
def _brain_q():
    ...
    try:
        ...
        """)
        conn.commit()
        cur.execute("SELECT id, question, answer FROM brain_quiz ORDER BY id DESC LIMIT 1")
        row = cur.fetchone()
        cur.close(); conn.close()
        return {"id": row[0], "q": row[1], "a": row[2]} if row else None
    except Exception as e:
        logger.error(f"_brain_q: {e}")
        return None

def _brain_st(user_id):
    """Bugungi urinish holati"""
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(
        ...
        row = cur.fetchone()
        cur.close(); conn.close()
        return {"n": row[0], "ok": row[1]} if row else {"n": 0, "ok": False}
    except:
        return {"n": 0, "ok": False}

def _brain_save(user_id, is_correct):
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("""
...
async def brain_menu(message: types.Message, state: FSMContext):
    ...
    if st["ok"]:
        await message.answer(
            f"🧩 <b>Miyya charxlash</b>\n\n"
            f"❓ {q[\'q\']}\n\n"
            "✅ Bugun to\'g\'ri javob berdingiz! +50 ball 🎉\n"
            "Ertaga yangi savol!"
        )
        return
    ...
    if st["n"] >= 2:
        await message.answer(
            f"🧩 <b>Miyya charxlash</b>\n\n"
            f"❓ {q[\'q\']}\n\n"
            "❌ Bugun 2 ta urinishdan foydalandingiz\n"
            f"💡 Javob: <tg-spoiler>{q[\'a\']}</tg-spoiler>\n"
            "Ertaga yangi savol 💪"
        )
        return
    ...
    await message.answer(
        f"🧩 <b>Miyya charxlash</b>\n\n"
        f"❓ <b>{q[\'q\']}</b>\n\n"
        f"✍️ Javob yozing ({qolgan}/2 urinish qoldi)\n"
        "💡 To\'g\'ri javob uchun <b>+50 ball!</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def brain_answer(message: types.Message, state: FSMContext):
    uid = message.from_user.id
    ...
    if st["ok"] or st["n"] >= 2:
        await state.clear()
        return
    correct = (q["a"].strip().lower() == user_ans)
    _brain_save(uid, correct)
    if correct:
        add_score(uid, 50)
        await state.clear()
        await message.answer(
            "🎉 <b>To\'g\'ri javob!</b>\n"
            f"✅ Javob: <b>{q[\'a\']}</b>\n"
            "⭐ <b>+50 ball</b> qo\'shildi!",
            reply_markup=main_kb(uid)
        )
    else:
        st2 = _brain_st(uid)
        if st2["n"] < 2:
            await message.answer("❌ Noto\'g\'ri! Yana <b>1 ta</b> urinish qoldi 👇")
        else:
            await state.clear()
            await message.answer(
...
# Admin: savol boshqaruvi (/ siz — oddiy xabar)
@dp.message(F.text.lower() == "savol qo\'sh")
async def brain_adm_add(message: types.Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    ...
    await message.answer(
        ...
        "<code>Savol matni\nJavob matni</code>\n\n"
        "<b>Misol:</b>\n"
        "<code>Qaysi hayvon uxlamasdan suzadi?\nAkula</code>"
    )


@dp.message(BrainAdminState.waiting_question)
async def brain_adm_save(message: types.Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await state.clear()
        return
    ...
    if len(parts) < 2 or not parts[0].strip() or not parts[1].strip():
        await message.answer(
            "❌ Format xato! Ikki qatorda yozing:\n"
            "<code>Savol\nJavob</code>"
        )
        return
    ...
    try:
        ...
        cur.execute(
            ...
            (q_text, a_text, message.from_user.id)
        )
        conn.commit()
        cur.close(); conn.close()
        await state.clear()
        await message.answer(f"✅ Savol qo\'shildi!\n❓ {q_text}\n💡 {a_text}")
    except Exception as e:
        await state.clear()
        await message.answer(f"❌ Xato: {e}")


@dp.message(F.text.lower() == "savol ro\'yxat")
async def brain_adm_list(message: types.Message):
    if message.from_user.id not in ADMIN_IDS:
        return
    try:
        conn = get_conn()
...
@dp.message(F.text.lower().startswith("savol o\'chir"))
async def brain_adm_del(message: types.Message):
    if message.from_user.id not in ADMIN_IDS:
        return
    parts = message.text.strip().split()
    if len(parts) < 2 or not parts[-1].isdigit():
        await message.answer("Format: <code>savol o\'chir 5</code>")
        return
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("DELETE FROM brain_quiz WHERE id=%s", (int(parts[-1]),))
        conn.commit()
        cur.close(); conn.close()
        await message.answer(f"✅ #{parts[-1]} o\'chirildi.")
    except Exception as e:
        await message.answer(f"❌ {e}")
