...
async def check_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if q_num >= total:
        ...
        # Daraja bo'yicha maslahat
        if _level == "beginner":
            level_tip = (
                f"\n💡 <b>Diqqat:</b> Oddiy darajada har to'g'ri javob uchun faqat <b>+{_ball} ball</b>!\n"
                f"⬆️ Ko'proq ball yig'ish uchun <b>O'rta yoki Yuqori darajaga</b> o'ting (+20/+35 ball)!"
            )
        else:
            level_tip = ""
...
async def grammar_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if is_correct:
        correct_count += 1
        _user_g  = get_user(user_id)
        _level_g = _user_g.get("level", "beginner") if _user_g else "beginner"
        _ball_g  = LEVEL_TEST_BONUS.get(_level_g, 10)
        add_score(user_id, _ball_g)
        explanation = questions[q_index].get("explanation", "")
        await callback.answer(f"✅ To'g'ri! +{_ball_g} ball 🎉\n{explanation}")
        await _check_score_prize(user_id)
    else:
        explanation = questions[q_index].get("explanation", "")
        await callback.answer(
...
def _update_battle_result(user_id: int, won: bool):
    """Bellashuv natijasini saqlaydi, streak yangilaydi"""
    stats = USER_BATTLE_STATS.setdefault(user_id, {"wins": 0, "losses": 0, "streak": 0})
    if won:
        stats["wins"] += 1
        stats["streak"] += 1
    else:
        stats["losses"] += 1
        stats["streak"] = 0  # Yutqizsa streak tushadi
