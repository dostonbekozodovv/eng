...
async def _send_battle_question(uid: int, battle: dict):
    ...
    # Raqib ismini aniqlash
    if battle.get("is_bot_battle"):
        opp_name = battle.get("bot_name", "Raqib")
    else:
        opp_uid  = uid2 if uid == uid1 else uid1
        opp_u    = get_user(opp_uid)
        opp_name = (opp_u or {}).get("name", "Raqib") if opp_u else "Raqib"

    # Tark etish tugmasi + javob tugmalari
    leave_btn = InlineKeyboardButton(text="🚪 Tark etish", callback_data=f"btl_leave_{bid}_{uid}")
    full_kb   = InlineKeyboardMarkup(inline_keyboard=[
        *kb.inline_keyboard,
        [leave_btn]
    ])
    ...
    if uid == BOT_USER_ID:
        return  # Botga xabar yuborilmaydi
    try:
        msg = await bot.send_message(uid, txt, reply_markup=full_kb)
...
async def _finish_battle(bid: str, reason: str = "complete", winner_id: int = None, loser_id: int = None):
    ...
    if reason == "leave_random":
        # Chiqib ketganga -100, g'olibga +100
        if w_id:
            add_score(w_id, BATTLE_WIN_BALL)
            add_score(l_id, BATTLE_LEAVE_BALL)
    elif reason == "leave_friend":
        # Do'st bellashuvida chiqib ketganga ball yo'q, g'olibga +100
    ...
    # Xabar yuborish
    async def notify(uid, my_s, opp_s, is_winner, is_draw, is_leave_penalty):
        if is_draw:
            emoji = "🤝"
            result = "DURRANG!"
            ball_txt = "±0 ball"
        ...
        try:
            await bot.send_message(
                ...
                f"{emoji} <b>{result}</b>\n\n"
                f"📊 <b>Hisobot:</b>\n"
                f"  🔵 Sizning ballingiz: <b>{my_s}</b>\n"
                f"  🔴 Raqib bali: <b>{opp_s}</b>\n\n"
                f"💰 {ball_txt}\n\n"
    ...
    if is_bot_battle:
        bot_name = battle.get("bot_name", "Raqib")
        if is_draw:
            emoji2 = "🤝"; result2 = "DURRANG!"; ball2 = "±0 ball"
        elif u1_wins:
            emoji2 = "🏆"; result2 = "G'ALABA!"; ball2 = f"+{BATTLE_WIN_BALL} ball"
        else:
            emoji2 = "😔"; result2 = "MAG'LUBIYAT"; ball2 = f"{BATTLE_LOSE_BALL} ball"
        try:
            await bot.send_message(
                uid1,
                f"⚔️ <b>BELLASHUV TUGADI!</b>\n\n"
                f"{emoji2} <b>{result2}</b>\n\n"
                f"📊 <b>Hisobot:</b>\n"
                f"  🔵 Sizning ballingiz: <b>{s1}</b>\n"
                f"  🔴 {bot_name}: <b>{s2}</b>\n\n"
                f"💰 {ball2}\n\n"
                f"🔙 Qayta o'ynash: ⚔️ tugmasini bosing"
            )
        ...
        await notify(uid1, s1, s2, u1_wins, is_draw, u1_leave_pen)
        if uid2 != BOT_USER_ID:
            await notify(uid2, s2, s1, u2_wins, is_draw, u2_leave_pen)
...
# ── O'zbek ismlari (bot ekanligini bildirmaydi) ──────────────────────────
_BOT_NAMES = [
    "Jasur","Sherzod","Bobur","Nodir","Sardor","Ulugbek","Doniyor","Firdavs",
    "Otabek","Mirzo","Sanjar","Eldor","Murod","Temur","Jahongir","Behruz",
    "Nilufar","Malika","Dilnoza","Feruza","Mohira","Zulfiya","Shahnoza",
    "Gulnora","Nafisa","Maftuna","Barno","Yulduz","Dilfuza","Kamola",
]
_used_bot_names: set = set()

def _pick_bot_name() -> str:
    global _used_bot_names
    available = [n for n in _BOT_NAMES if n not in _used_bot_names]
    if not available:
        _used_bot_names.clear()
        available = list(_BOT_NAMES)
    name = random.choice(available)
    _used_bot_names.add(name)
    return name

# Bot profillari (daraja + aniqlik, 4-5 sekund kechikish)
_BOT_PROFILES = [
    {"level": "beginner",     "accuracy": 0.50},
    {"level": "beginner",     "accuracy": 0.58},
    {"level": "intermediate", "accuracy": 0.65},
    {"level": "intermediate", "accuracy": 0.72},
    {"level": "advanced",     "accuracy": 0.82},
]
BOT_USER_ID = -999999

def _pick_bot_profile(user_id: int) -> dict:
    user  = get_user(user_id)
    level = (user or {}).get("level", "beginner")
    if level == "beginner":
        pool = _BOT_PROFILES[0:2]
    elif level == "intermediate":
        pool = _BOT_PROFILES[1:4]
    else:
        pool = _BOT_PROFILES[3:5]
    chosen = dict(random.choice(pool))
    chosen["name"] = _pick_bot_name()
    return chosen

async def _run_bot_battle(bid: str, profile: dict):
    """Bot tomonidan 4-5 soniyada javob beradi"""
    ...
    while True:
        ...
        if not battle or battle.get("finished"):
            break
        # Bot hali javob bermaganini kutamiz
        if battle.get("ans2_done"):
            await asyncio.sleep(0.4)
            continue

        cur_q = battle.get("q_num", 1)
        # 4-5 sekund o'ylash
        await asyncio.sleep(random.uniform(4.0, 5.0))

        battle = ACTIVE_BATTLES.get(bid)
        if not battle or battle.get("finished") or battle.get("ans2_done"):
            continue

        if battle.get("q_num", 1) != cur_q:
            continue  # savol o'zgarib qoldi
...
async def _queue_timeout(user_id: int, msg_id: int, chat_id: int):
    ...
    # Bot bilan boshlash (foydalanuvchiga aytmaymiz)
    profile = _pick_bot_profile(user_id)
    pool    = _get_battle_pool(user_id)
    bot_lv  = words.get(profile["level"], words.get("beginner", []))[:40]
    pool    = pool + [w for w in bot_lv if w not in pool]
    if len(pool) < 4:
        pool = words.get("beginner", [])[:40]
    random.shuffle(pool)

    bid    = f"{user_id}_{BOT_USER_ID}"
    battle = {
        "battle_id": bid,
        "uid1": user_id, "uid2": BOT_USER_ID,
        "score1": 0,     "score2": 0,
        "q_num": 1,
        "ans1_done": False, "ans2_done": False,
        "pool": pool,
    ...
    await asyncio.sleep(1)
    await _send_battle_question(user_id, battle)
    asyncio.create_task(_run_bot_battle(bid, profile))
...
async def battle_leave_cb(callback: types.CallbackQuery):
    """O'yinchi bellashuvni tark etadi"""
    parts = callback.data.split("_")
    # btl_leave_{uid1}_{uid2}_{uid}
    if len(parts) < 5:
        await callback.answer()
        return
    ...
    try:
        await callback.message.edit_text("🚪 Bellashuvdan chiqdingiz.")
    except Exception:
        pass
    await callback.answer()

    if is_bot:
        # Bot bilan — faqat foydalanuvchiga xabar, -100 ball
        add_score(uid, BATTLE_LEAVE_BALL)
        ACTIVE_BATTLES.pop(bid, None)
        try:
            await bot.send_message(
                uid,
                f"🚪 <b>Bellashuvdan chiqdingiz!</b>\n\n"
                f"💸 <b>{BATTLE_LEAVE_BALL} ball</b> ayrildi."
            )
        except Exception:
            pass
    elif is_friend:
