...
from config import (
    ...
    WEEKLY_BONUS_PRICE   # ← config.py ga qo'shiladi
)
from db import (
...
# ══════════════════════════════════════════════════
# FIX 1: /stopquiz — quiz boshlagan user ham to'xtatsin
# ══════════════════════════════════════════════════
@dp.message(Command("stopquiz"))
async def cmd_stopquiz(message: types.Message):
    """Viktorinani to'xtatish — admin YOKI quiz boshlagan user"""
    ...
    # Tekshirish: admin yoki quiz boshlagan user
    started_by = qs.get("started_by")
    is_admin_user = False
    try:
    ...
    # Shaxsiy chatda ham ishlashi uchun (chat_id > 0)
    if chat_id > 0:
        is_admin_user = True  # shaxsiy chatda faqat o'zi bo'ladi

    if not is_admin_user and sender_id != started_by and sender_id not in ADMIN_IDS:
        await message.reply(
            "❌ Faqat <b>admin</b> yoki <b>viktorinani boshlagan</b> kishi to'xtatishi mumkin."
        )
        return
...
async def poll_group_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    # FIX 1: started_by ni saqlash — stopquiz uchun kerak
    GROUP_QUIZ_STATE[chat_id] = {
        ...
        "unanswered_streak": 0,
        "last_poll_id":      None,
        "started_by":        user_id,   # ← quiz boshlagan user
        "is_private":        chat_id > 0,  # ← shaxsiy chat belgisi
    }

    await callback.message.edit_text(
        f"🎯 <b>{group_num}-guruh viktorinasi boshlanmoqda!</b>\n\n"
        f"📊 Jami savollar: <b>{total}</b>\n"
        f"⏱ Har savol uchun <b>30 soniya</b>\n\n"
        f"🛑 To'xtatish uchun: /stopquiz\n\n"
        f"Hamma qatnashing! 🏆",
        reply_markup=None
    )
    await state.clear()
...
# ══════════════════════════════════════════════════
# FIX 2: Shaxsiy chatda poll to'g'ri ishlashi
# ══════════════════════════════════════════════════
async def _send_group_poll(chat_id: int):
    ...
    is_private        = qs.get("is_private", False)

    # Shaxsiy chatda unanswered_streak tekshirmaymiz —
    # foydalanuvchi o'zi javob beradi, bot navbat kutmaydi
    if not is_private and unanswered_streak >= 2 and poll_num > 0:
        qs["active"] = False
        scores = qs.get("scores", {})
        result_text = (
            f"⏹ <b>Viktorina to'xtatildi!</b>\n\n"
            f"😔 2 ta savolga hech kim javob bermadi.\n\n"
        )
        if scores:
            result_text += f"📊 <b>{group_num}-guruh — oraliq natijalar:</b>\n"
            result_text += f"Jami savollar: <b>{poll_num}/{total}</b>\n"
            result_text += "━━━━━━━━━━━━━━━━━━\n"
            sorted_scores = sorted(scores.items(), key=lambda x: x[1], reverse=True)
            medals = ["🥇", "🥈", "🥉"]
            for i, (uid, count) in enumerate(sorted_scores[:10]):
                ...
                try:
                    ...
                    name = f"Ishtirokchi {i+1}"
                result_text += f"{medal} <b>{name}</b> — {count}/{poll_num} ✅\n"
        result_text += "\n/quiz — yana boshlash"
    ...
    # Shaxsiy chat uchun open_period 30 sek, guruh uchun 20 sek
    open_period = 30 if is_private else 20

    sent = await bot.send_poll(
        chat_id=chat_id,
        question=f"❓ {poll_num + 1}/{total} — 🇬🇧 {correct['word'].upper()} tarjimasini toping!",
        options=options,
        type="quiz",
    ...
    # FIX 2 (asosiy):
    # Guruh: poll 20 sek + 2 sek pauza = 22 sek kutamiz
    # Shaxsiy: poll 30 sek tugagandan keyin avtomatik keyingi savol keladi
    #          lekin shaxsiy chatda javob berilganda darhol o'tkazish kerak
    #          shuning uchun delay = open_period + 2 (pauza)
    delay = (open_period + 2)
    asyncio.create_task(_delayed_next_poll(chat_id, poll_id, delay))


async def _finish_group_quiz(chat_id: int, qs: dict):
    """Viktorina natijasini chiqarish"""
    group_num = qs["group_num"]
    total     = qs["total"]
    scores    = qs.get("scores", {})
    qs["active"] = False
    ...
    if scores:
        ...
        result_text   = f"🏆 <b>{group_num}-guruh viktorinasi yakunlandi!</b>\n\n"
        result_text  += f"📊 Jami savollar: <b>{total}</b>\n"
        result_text  += "━━━━━━━━━━━━━━━━━━\n"
        medals = ["🥇", "🥈", "🥉"]
        for i, (uid, count) in enumerate(sorted_scores[:10]):
            medal = medals[i] if i < 3 else f"{i+1}."
            try:
                ...
                name = f"Ishtirokchi {i+1}"
            winner_tag = " 🏆 G'OLIB!" if i == 0 and count == max_score else ""
            result_text += f"{medal} <b>{name}</b> — {count}/{total} ✅{winner_tag}\n"
...
async def _delayed_next_poll(chat_id: int, poll_id: str, delay: int):
    ...
    # Bu poll hali ham "last_poll" ekanligini tekshirish
    # Agar boshqa poll kelgan bo'lsa — bu task eskirgan
    if qs.get("last_poll_id") != poll_id:
        return

    is_private = qs.get("is_private", False)

    # Shaxsiy chatda unanswered_streak hisoblamaymiz
    if not is_private:
        answered_users = qs.get("answered_polls", {}).get(poll_id, set())
...
# ══════════════════════════════════════════════════
# FIX 2b: Poll javob berilganda shaxsiy chatda darhol keyingisiga o'tish
# ══════════════════════════════════════════════════
@dp.poll_answer()
async def on_poll_answer(poll_answer: types.PollAnswer):
    """Poll javoblarini kuzatish"""
    ...
    for chat_id, qs in list(GROUP_QUIZ_STATE.items()):
        if poll_id in qs.get("poll_answers", {}):
            ...
            # Javob berganlarni ro'yxatga olish
            if "answered_polls" not in qs:
                qs["answered_polls"] = {}
            ...
            # FIX 2b: Shaxsiy chatda javob berilganda darhol (2 sek pauza bilan)
            # keyingi savolga o'tish — foydalanuvchi 30 sek kutmaydi
            if is_private and qs.get("active") and qs.get("last_poll_id") == poll_id:
                asyncio.create_task(_private_next_poll_after_answer(chat_id, poll_id))
            break


async def _private_next_poll_after_answer(chat_id: int, poll_id: str):
    """Shaxsiy chatda javob berilgandan 2 sek so'ng keyingi savol"""
    await asyncio.sleep(2)
    qs = GROUP_QUIZ_STATE.get(chat_id)
    if not qs or not qs.get("active"):
        return
    # Hali ham bir xil poll bo'lsa (boshqa task ishlamagan bo'lsa)
    if qs.get("last_poll_id") == poll_id:
        await _send_group_poll(chat_id)
...
async def show_top_ref(callback: types.CallbackQuery):
    ...
    # FIX 3: WEEKLY_BONUS_PRICE variable ishlatildi
    text += f"\n<i>Har juma 5+ taklif qilgan g'olib {WEEKLY_BONUS_PRICE:,} so'm yutadi! 🏆</i>"
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def referal_menu(message: types.Message):
    ...
    await message.answer(
        ...
        f"• G'olibga <b>💰 {WEEKLY_BONUS_PRICE:,} so'm</b>!\n\n"
        ...
        try:
            ...
            if top_list:
                ...
                if winner.get("referral_count", 0) >= MIN_REFERRALS_FOR_BONUS:
                    # FIX 3: WEEKLY_BONUS_PRICE variable ishlatildi
                    add_referral_earnings(winner["user_id"], WEEKLY_BONUS_PRICE)
                    try:
                        await bot.send_message(
                            winner["user_id"],
                            f"🎉 <b>HAFTALIK G'OLIB!</b> 🏆\n\n"
                            f"👥 Taklif: <b>{winner['referral_count']} ta</b>\n"
                            f"💰 <b>+{WEEKLY_BONUS_PRICE:,} so'm</b> bonus!\n\n"
                            f"Tabriklaymiz! 🎊"
                        )
                    except Exception:
                        pass
...
async def main():
    ...
    group_commands = [
        BotCommand(command="start",    description="🚀 Bot haqida / komandalar"),
        BotCommand(command="quiz",     description="🎯 So'z viktorinasi (Poll)"),
        BotCommand(command="stopquiz", description="⏹ Viktorinani to'xtatish"),
        BotCommand(command="test",     description="🧠 Shaxsiy test ishlash"),
        BotCommand(command="learn",    description="📚 So'z o'rganish (shaxsiy)"),
        BotCommand(command="streak",   description="🔥 Streak ko'rish"),
        BotCommand(command="rating",   description="🏆 Reyting"),
    ]
