...
async def referal_menu(message: types.Message):
    ...
    share_text = f"🎁 Bu botda ingliz tilini o'rganmoqdaman! Sen ham ko'r: {ref_link}"
    bot_info2 = await bot.get_me()
    friend_link = f"https://t.me/{bot_info2.username}?start=friend_{user_id}"
    await message.answer(
        ...
        f"━━━━━━━━━━\n"
        f"⚔️ <b>DO'ST BILAN BELLASHISH:</b>\n"
        f"Do'stingizni bellashuvga taklif qilish:\n"
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(
                text="⚔️ Do'st bellashuvga taklif",
                callback_data="battle_invite_link"
            )],
        ])
    )
...
async def battle_friend_menu_cb(callback: types.CallbackQuery):
    ...
    kb_rows.append([InlineKeyboardButton(
        ...
        switch_inline_query=f"Do'st bilan inglizcha bellashuvga taklif qilaman! {inv_link}"
    )])
...
async def battle_invite_link_cb(callback: types.CallbackQuery):
    ...
    user_id  = callback.from_user.id
    bot_info = await bot.get_me()
    token    = f"fb_{user_id}_{random.randint(10000,99999)}"
    inv_link = f"https://t.me/{bot_info.username}?start=fbattle_{user_id}"
    FRIEND_BATTLE_INVITES[token] = {"inviter_id": user_id, "accepted": False}
    await callback.message.answer(
        f"⚔️ <b>DO'ST BELLASHUV HAVOLASI</b>\n\n"
        f"Ushbu havolani do'stingizga yuboring:\n"
        f"<code>{inv_link}</code>\n\n"
        f"U bosib kirsa, avtomatik bellashuvga ulanadi!"
    )
    await callback.answer()
