...
# ══════════════════════════════════════════════════
# /quiz — GURUH VIKTORINASI (Telegram Poll)
# ══════════════════════════════════════════════════
...
async def cmd_quiz(message: types.Message):
    ...
    if len(all_words) < 4:
        await message.reply("❌ So'zlar yetarli emas.")
        return

    # To'g'ri javob va 3 ta noto'g'ri variant
    ...
    await message.answer_poll(
        question=f"🇬🇧 {correct['word'].upper()} — tarjimasini toping!",
        options=options,
        type="quiz",
        correct_option_id=correct_idx,
        explanation=f"💡 Misol: {correct['example']}",
        is_anonymous=False,
        allows_multiple_answers=False,
        reply_markup=ReplyKeyboardRemove() if not is_group(message) else None
    )
